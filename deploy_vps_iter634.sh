#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 634)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 634) ═══════════
#
# 💾 AUTO-SAVE EVERY MINUTE (user request — "rollback the autosave"):
#  * Compliance Salary Process: while a run has unsaved edits, the full
#    sheet is silently persisted every 60 seconds (same as Save as
#    Draft). A green "✓ Auto-saved HH:MM:SS" indicator replaces the
#    amber unsaved warning after each autosave. Finalized runs are
#    never touched; failed autosaves retry the next minute.
#  * Actual Salary Process: already saves every edit instantly (450 ms
#    debounced PATCH); failed saves are now re-queued and RETRIED after
#    1 minute instead of being lost.
#
# 🗂  MULTI-TAB FIX — NO MORE REFRESH / REDIRECT TO DASHBOARD:
#  * ROOT CAUSE FOUND: the Iter 614 PWA auto-updater re-checked the
#    server version every time a tab became VISIBLE and force-reloaded
#    the tab after any deploy — losing unsaved work and landing on the
#    Dashboard. The update check now runs ONLY at app launch; switching
#    tabs never reloads the app. (New versions still apply on the next
#    fresh open.)
#  * UNSAVED-CHANGES GUARD: refreshing or closing a tab with unsaved
#    compliance-sheet edits now shows the browser "leave site?"
#    confirmation.
#
# 🔢 Iter 633 (also included) — whole-rupee calculation + Excel
#    (Displayed) · 632 — whole-rupee exports · 631 — disable warning.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy634.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy634.sh
...
echo -n "   Server badge is 634 (must say OK): "
grep -q 'APP_ITERATION = "634"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 634 deployed."
echo "  • NEW (634): AUTO-SAVE RESTORED — the Compliance Salary sheet"
...
echo "  • NEW (634): MULTI-TAB FIX — switching back to an older browser"
