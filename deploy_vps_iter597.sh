#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 597 — CONTRACTOR PF CALCULATION RULE in PF Settings)
# Deploys the FULL latest code (includes ALL of Iter 568-596). Running 597 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 597) ═══════════
#
# 🏗️ CONTRACTOR PF CALCULATION RULE (PF/ESIC Settings — global OR per-firm):
#  * New setting "Contractor PF Calculation Rule":
#      Standard EPF Calculation (default) | Contractor Wage-Based PF.
#  * Wage-Based mode — PF strictly on the EARNED Basic+DA of the period:
#      - Full period, earned ≥ ₹15,000 ceiling → PF wage = ceiling → PF ₹1,800 (Rule 2)
#      - Partial period (LOP / fewer days / mid-month join or exit) →
#        PF on the ACTUAL earned PF wage — e.g. ₹18,000 basic, 12 of 30
#        days → earned ₹7,200 → PF ₹864; employer EPF/EPS split on the
#        same wage with EPS ceiling as before (Rule 3)
#      - The 50% wage-definition floor does NOT apply in this mode (Rule 1)
#  * Sub-setting "During Partial Month" for Adopt-PF employees (Rule 4):
#      Adopted/Fixed PF Wage (Company Policy, default) — contributions stay
#      on the full adopted wage regardless of attendance · OR
#      Actual Earned PF Wage (Recommended) — adopted wage pro-rated by days.
#  * Reverse / Freeze-gross firms: Freeze Gross stays UNCHANGED — only the
#      PF wage follows these rules (Rule 5).
#  * PF/ESIC Audit "pf_reason" now names the exact contractor rule applied;
#      grid Present-Days/OT edits recompute with the same contractor logic.
#
# PREVIOUS (Iter 596) — Challan Uploads: Month + Employee Group auto-select,
#    "All groups" merges every finalized run of the month into one file.
# PREVIOUS (Iter 595) — header firm dropdown pins CURRENT FIRM; keyboard
#    Phase 2 (grid ↑↓←→/Enter/P/A quick marks, F5 salary recalculate).
# PREVIOUS (Iter 587-594) — RBAC phases, Maker-Checker, AI Command Center,
#    permission matrix, 2FA OTP login, shortcuts Phase 1.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy597.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy597.sh
...
echo -n "   Server badge is 597 (must say OK): "
grep -q 'APP_ITERATION = "597"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
