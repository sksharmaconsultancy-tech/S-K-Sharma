#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 598 — FIRMS ID & PASSWORD: SUB SUPER ADMIN PIN ACCESS FIXED)
# Deploys the FULL latest code (includes ALL of Iter 568-597). Running 598 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 598) ═══════════
#
# 🔑 FIRMS ID & PASSWORD — SUB SUPER ADMIN PIN ACCESS:
#  * Sub Super Admins unlock the vault with THEIR OWN 6-digit PIN (the
#      screen now says so explicitly — no more confusion with the Super
#      Admin's PIN).
#  * Sub Super Admins created WITHOUT a PIN used to see a generic
#      "wrong PIN" error → now a clear message explains exactly what to do.
#  * NEW "Forgot PIN? Email me a temporary PIN" button right on the
#      Verify-PIN card — emails a temp PIN to the signed-in admin (works
#      even when NO PIN was ever set); they pick a personal PIN on next
#      sign-in. Super Admin can also set/replace any Sub Admin's PIN in
#      Sub Admins → Edit → 6-Digit PIN.
#
# PREVIOUS (Iter 597) — Contractor PF Calculation Rule in PF Settings
#    (wage-based earned Basic+DA, partial-month policy, per-firm override).
# PREVIOUS (Iter 596) — Challan Uploads: Month + Employee Group auto-select,
#    "All groups" merges every finalized run of the month.
# PREVIOUS (Iter 595) — CURRENT FIRM pinned in header picker; keyboard
#    Phase 2 (grid arrows/Enter/P/A, F5 salary recalculate).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy598.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy598.sh
...
echo -n "   Server badge is 598 (must say OK): "
grep -q 'APP_ITERATION = "598"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
