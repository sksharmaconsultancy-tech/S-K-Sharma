#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 729)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 729) — ✅ KAMESHVAR CASE: NIGHT-SHIFT DOUBLE-SCAN अब सही ═══════════
#
...
#   wget -O deploy729.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy729.sh
...
grep -q 'APP_ITERATION = "729"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
