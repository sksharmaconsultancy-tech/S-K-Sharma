#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 693)
...
#   wget -O deploy693.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy693.sh
...
echo "==> Iter 693 report: firms sharing the SAME EPFO login (read-only — nothing is changed)..."
$APP_DIR/backend/venv/bin/python - << 'PYDUP' || echo "⚠ duplicate report skipped"
import asyncio, os
...
async def m():
    ...
    for u, cids in dups.items():
        print("   ⚠ EPFO User ID '%s' in %d firms me saved hai: %s" % (
            u, len(cids), ", ".join(names.get(c, c) for c in cids)))
        print("     → Jis firm ka ye login NAHI hai, uske Firm Master me sahi login daal kar Save karein.")
asyncio.run(m())
PYDUP
...
echo -n "   Server badge is 693 (must say OK): "
grep -q 'APP_ITERATION = "693"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q '_dup_epfo_login_warning' $APP_DIR/backend/routes/portal_extension.py && grep -q 'EPFO_PC_ACTION' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'GuardedGridInput' $APP_DIR/frontend/app/firm-master.tsx && grep -q 'RUNNER_VERSION = "22"' $APP_DIR/backend/routes/portal_extension.py && echo "OK" || echo "MISSING!"
