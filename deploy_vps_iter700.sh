#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 700)
...
#   wget -O deploy700.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy700.sh
...
echo -n "   Server badge is 700 (must say OK): "
grep -q 'APP_ITERATION = "700"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q '_dup_epfo_login_warning' $APP_DIR/backend/routes/portal_extension.py && grep -q 'EPFO_PC_ACTION' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'GuardedGridInput' $APP_DIR/frontend/app/firm-master.tsx && grep -q 'RUNNER_VERSION = "26"' $APP_DIR/backend/routes/portal_extension.py && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "26"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v18' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'typeof body === "string" ? body' $APP_DIR/frontend/src/api/client.ts && grep -q 'sks-pwa-v18' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'ecr_attached' $APP_DIR/backend/routes/portal_extension.py && grep -q 'ecr_attached' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'RUNNER_VERSION = "26"' $APP_DIR/backend/routes/portal_extension.py && grep -q '"label": "ECR Upload"' $APP_DIR/backend/utils/rpa_engine.py && echo "OK" || echo "MISSING!"
echo -n "   ESIC same login process v26 — Iter 700 (must say OK): "
grep -q 'esic_open' $APP_DIR/backend/routes/portal_extension.py && grep -q '_diagnose_esic_creds' $APP_DIR/backend/routes/portal_extension.py && grep -q 'esic_open' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'sks-pwa-v18' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
