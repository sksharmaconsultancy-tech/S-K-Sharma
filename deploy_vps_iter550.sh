#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 550)
#
# ═══════════ WHAT'S NEW SINCE 549 (this deploy) ═══════════
#
# A. "OT —" DASHES CLEANED UP (user question):
#    Some employees (e.g. GANESH RAM PRAJAPAT 03/04-Aug) showed
#    "OT — / —" in the day cell. That OT is CORRECT — it was earned
#    INSIDE the single duty punch pair (worked longer than the duty
#    quota, e.g. 08:01→20:21 = 12:20 → 12:00 duty + 00:30 OT). There
#    are no separate OT punches, so no OT In/Out times exist.
#    Now such days show a clean single amber line instead of dashes:
#        08:01 / 20:21 / 12:00 / +OT 00:30 / Tot 12:30
#    Days with real OT punches keep the full OT In/Out lines.
#    Same cleanup applied to the IN/OUT PDF export.
#
#    HOW TO CHECK AFTER DEPLOY: Attendance Report → IN / OUT →
#    GANESH RAM PRAJAPAT → 03 Aug cell shows "+OT 00:30" (no dashes).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy550.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy550.sh
...
echo -n "   Server badge is 550 (must say OK): "
grep -q 'APP_ITERATION = "550"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo "CHECK: GANESH RAM PRAJAPAT 03-Aug cell shows +OT 00:30 (no OT — dashes)."
