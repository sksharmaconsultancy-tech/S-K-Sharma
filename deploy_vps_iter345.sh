#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 345)
# Ships (on top of everything listed below from 344):
#   • ALL-FIRMS SALARY STRUCTURE SYNC (user: "So many companies facing
#     issues regarding allowances/salary not fetched from Old DB"):
#     Legacy Import screen now has a one-click button
#     "Sync Salary Structures — ALL Firms (from Old DB)". It re-reads the
#     head-wise structure (Basic + allowances) for EVERY imported firm and
#     updates Basic / Allowances / Gross / Monthly Salary on existing
#     employees (matched by employee code). Live progress + per-firm errors.
...
echo ""
echo "✅ Deploy Iter 345 complete! HARD-REFRESH the browser (Ctrl+Shift+R)."
