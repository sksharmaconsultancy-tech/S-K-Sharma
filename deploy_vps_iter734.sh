#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 734)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 734) — 🔍 SUNSHINE SUITINGS SALARY PROCESS ISSUE ═══════════
#
...
#   wget -O deploy734.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy734.sh
...
grep -q 'APP_ITERATION = "734"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
