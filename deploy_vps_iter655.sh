#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 655)
...
#   wget -O deploy655.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy655.sh
...
echo "==> 8b/9 Nginx hardening for BIG salary sheets (Iter 655)..."
...
echo "==> 8c/9 SPEED-UP: nginx compression, caching & HTTP/2 (Iter 655)..."
...
# S.K. Sharma & Co. — performance tuning (deploy Iter 655)
gzip on;
gzip_comp_level 5;
...
echo -n "   Server badge is 655 (must say OK): "
grep -q 'APP_ITERATION = "655"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 655 deployed."
echo "  • NEW (655): SPEED-UP — pre-compressed bundles, gzip_static,"
