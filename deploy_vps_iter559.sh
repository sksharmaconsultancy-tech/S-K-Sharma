#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 559 — includes 555→558)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 559) ═══════════
#
# A. PUNCH APPROVALS — EXCEL EXPORT [Iter 559 NEW]:
#    • Green "Excel" button next to Show/Save on the source tabs
#      (Updated / Auto-Punches / Manual Entries / Additional Duty).
#    • Exports the current tab with the SAME columns as on screen:
#      Date, Code, Bio Code, Name, Father Name, Designation,
#      In/Out Punch, Duty HRS, OT In/Out, OT HRS, Total HRS.
#    • Respects the date (single day or period) and the search box —
#      supervisors can circulate correction lists to site managers.
#
# B. PUNCH APPROVALS columns fixed + Bio Code column [Iter 558].
# C. TIME FORMAT dropdown (12 Hr AM/PM default) in Daily Verification
#    + exports [Iter 557].
# D. MANUAL OT PUNCH EDIT BUG FIXED (Anil Purbiya 5th) [Iter 555].
# E. 2 MORE SUPER ADMINS survive restarts [Iter 555].
# F. "ADMIN CORRECTED" ✎ badge on attendance grid [Iter 556].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy559.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy559.sh
...
echo -n "   Server badge is 559 (must say OK): "
grep -q 'APP_ITERATION = "559"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Excel export endpoint (must say OK): "
grep -q 'attendance_day_status_export' $APP_DIR/backend/routes/attendance_admin_core.py && echo "OK" || echo "MISSING!"
echo -n "   Excel button in Punch Approvals UI (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 559 deployed."
echo "  Test: Punch Approvals → Auto-Punches → pick date → Show →"
echo "    green 'Excel' button downloads the tab with Bio Code column."
echo "  Plus all Iter 555–558 items if not yet verified."
