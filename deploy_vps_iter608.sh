#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 608 — Test-machine cleanup + Super Admin full access)
# Deploys the FULL latest code (includes ALL of Iter 568-607). Running 608 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 608) ═══════════
#
...
#   wget -O deploy608.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy608.sh
...
echo -n "   Server badge is 608 (must say OK): "
grep -q 'APP_ITERATION = "608"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 608 deployed."
echo "  • Biometric Devices: PROBE-*/TEST-* entries are gone; use the new"
echo "    'Dismiss' button for any other stray serial."
echo "  • CN4C231160062 pinged your server 8000+ times — that looks like a"
echo "    REAL machine. Register it if it's yours."
echo "  • Super Admin now bypasses all firm-level feature locks."
