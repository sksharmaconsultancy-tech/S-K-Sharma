#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 564 — includes 555→563)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 564) ═══════════
#
# A. API URL FOR THE CLIENT [Iter 564 NEW]:
#    • API Integration screen now shows the EXACT endpoint URL in a
#      green banner (with copy button):
#         POST https://<your-real-domain>/api/v1/punching
#      — it automatically uses your live portal domain.
#    • "Copy Vendor Docs" now embeds the real URL too, so the document
#      you send to the client is ready to use as-is.
#
# B. Security-failure email alerts [563], Test Console [562], Secure
#    Punching Push API + Integration Master + Logs + Docs [561],
#    Performance-sheet import [560], Punch Approvals Excel [559] +
#    Bio Code [558], Time Format 12/24 [557], ✎ badge [556],
#    Manual OT fix + Super Admins [555].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy564.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy564.sh
...
echo -n "   Server badge is 564 (must say OK): "
grep -q 'APP_ITERATION = "564"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Real-URL vendor docs (must say OK): "
grep -q 'Iter 564 (user request)' $APP_DIR/backend/routes/punch_push_api.py && echo "OK" || echo "MISSING!"
echo -n "   API URL banner in UI (must say OK): "
grep -q 'api-endpoint-url' $APP_DIR/frontend/app/punching-api.tsx && echo "OK" || echo "MISSING!"
echo -n "   Security alerts — Iter 563 (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 564 deployed."
echo "  Administration → API Integration → green banner shows the"
echo "  exact URL to give the vendor: POST https://<your-domain>/api/v1/punching"
echo "  'Copy Vendor Docs' embeds the same real URL."
