#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 633)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 633) ═══════════
#
# 🔢 ALWAYS CALCULATE IN ROUND FIGURES (user request):
#  * The compliance engine now rounds EVERY money figure to a whole
#    rupee at calculation time — Monthly Gross, OT, Gross Paid, Stat/
#    PF/ESIC wage bases, PF (EE/EPF/EPS/Total/VPF), ESIC (EE/ER), PT,
#    TDS, master deductions — and RE-DERIVES Total Deduction and Net
#    from the rounded parts so every column always tallies exactly.
#  * Statutory percentages are still applied on the precise values
#    first (correct math), then rounded — so a Reprocess never brings
#    decimals back (e.g. 2903.85 is now stored/displayed as 2904).
#
# 📤 EXPORT THE DISPLAYED SHEET BEFORE SAVE (user request):
#  * New "Excel (Displayed)" button next to the Excel button on the
#    Compliance Salary sheet — it exports EXACTLY what is currently on
#    screen, INCLUDING edits not yet saved as draft (button shows a *
#    when there are unsaved changes). Whole-rupee formatting applies;
#    the export never writes anything to the database.
#
# 💰 Iter 632 (also included) — whole-rupee Excel/CSV exports ·
#    631 — allowance disable warning · 630 — allowance mask contract.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy633.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy633.sh
...
echo -n "   Server badge is 633 (must say OK): "
grep -q 'APP_ITERATION = "633"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 633 deployed."
echo "  • NEW (633): WHOLE-RUPEE CALCULATION — the compliance engine"
...
echo "  • NEW (633): EXCEL (DISPLAYED) button on the Compliance sheet"
