#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 565 — includes 555→564)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 565) ═══════════
#
# A. BULK EMPLOYEE IMPORT — DUPLICATE NAME PROTECTION [Iter 565 NEW]:
#    • Rows whose NAME already exists in the firm are SKIPPED by
#      default (case/space-insensitive: "Mahaveer  Singh" = "MAHAVEER
#      SINGH"). Also catches the same name typed twice INSIDE the sheet.
#    • New checkbox "Allow duplicate names" in the Import screen —
#      duplicates import ONLY with this explicit permission.
#    • Skip log clearly says why each row was skipped.
#
# B. API URL banner + real-URL vendor docs [564], security-failure
#    email alerts [563], Test Console [562], Secure Punching Push API
#    [561], Performance-sheet import [560], Punch Approvals Excel [559]
#    + Bio Code [558], Time Format 12/24 [557], ✎ badge [556],
#    Manual OT fix + Super Admins [555].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy565.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy565.sh
...
echo -n "   Server badge is 565 (must say OK): "
grep -q 'APP_ITERATION = "565"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Duplicate-name guard (must say OK): "
grep -q 'allow_duplicate_names' $APP_DIR/backend/routes/employees_admin.py && echo "OK" || echo "MISSING!"
echo -n "   Allow-duplicates checkbox in UI (must say OK): "
grep -q 'ebi-allow-dup-names' $APP_DIR/frontend/app/employee-bulk-import.tsx && echo "OK" || echo "MISSING!"
echo -n "   API URL banner — Iter 564 (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 565 deployed."
echo "  Test: Import/Export → Employee Bulk Import → sheet containing"
echo "    an existing employee's name → row SKIPPED with clear reason;"
echo "    tick 'Allow duplicate names' → imports."
