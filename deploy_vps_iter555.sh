#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 555)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555) ═══════════
#
# A. MANUAL OT PUNCH EDIT BUG — FIXED (your P0, Anil Purbiya 5th):
#    Root cause found: manually edited/added punches (source
#    "manual_admin") were being deleted by the 5-minute duplicate-punch
#    filter. The repair modal saves the OT IN 1 minute after the duty
#    OUT — when the duty OUT was ALSO manually repaired, the same-source
#    5-min rule silently dropped the OT IN, the OT OUT lost its pair,
#    and the whole manual OT session vanished from the reports.
#    FIX: manual admin punches are NEVER deduplicated (they are
#    deliberate corrections). Machine double-punch cleanup unchanged.
#    → After deploy, re-open Anil Purbiya 5th — the edited OT In/Out
#      punches will show again (no data was lost; punches are in the DB).
#
# B. 2 MORE SUPER ADMINS — NOW POSSIBLE:
#    Root cause found: a hidden startup sweep demoted ANY super admin
#    not in the hard-coded allowlist back to "employee" on every backend
#    restart — new super admins never survived.
#    FIX: accounts created via Administration → Super Admin Rights are
#    flagged exempt and keep super_admin permanently.
#    ALSO: the super-admin list API no longer exposes PIN hashes.
#    HOW TO ADD YOUR 2 SUPER ADMINS AFTER DEPLOY:
#      1. Administration → Super Admin Rights → Add
#      2. Enter name + email (+ password) → Save
#      3. They log in with email + password (forced to change password
#         on first login). Reset / disable / delete from the same screen.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy555.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy555.sh
...
echo -n "   Server badge is 555 (must say OK): "
grep -q 'APP_ITERATION = "555"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Manual OT dedupe fix (must say OK): "
grep -q 'Iter 555 (user bug' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Punch-sequence manual exemption (must say OK): "
grep -q 'manual_admin punches (deliberate admin repairs) are never' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo -n "   Super admin create flag (must say OK): "
grep -q 'super_admin_allowlisted' $APP_DIR/backend/routes/super_admins.py && echo "OK" || echo "MISSING!"
echo -n "   Super admin list hash-leak fix (must say OK): "
grep -q 'whitelist projection' $APP_DIR/backend/routes/super_admins.py && echo "OK" || echo "MISSING!"
echo -n "   Iter 554 AM/PM option still present (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 555 deployed."
echo "  Test 1 (OT punch fix): open Anil Purbiya 5th in the attendance"
echo "    grid / Daily Verification — the manually edited OT In/Out now"
echo "    shows. Re-edit any OT punch and confirm it stays."
echo "  Test 2 (Super Admins): Administration → Super Admin Rights →"
echo "    Add — create your 2 new super admins (email + password)."
echo "    They stay super admins even after server restarts now."
