#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 489)
#
# NEW IN 489 — COMPLIANCE SALARY REGISTER PDF: ADVANCE COLUMN FIX
...
echo -n "   Server Version badge shows 489 (must say OK): "
grep -q 'APP_ITERATION = "489"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "✅ Deploy Iter 489 complete! HARD-REFRESH the browser (Ctrl+Shift+R)."
echo ""
echo "   HOW TO VERIFY:"
echo "   1. Footer badge must read 'Server Iter 489'."
echo "   2. ADVANCE FIX: Compliance Salary → download the Salary Register"
echo "      PDF (both formats) — Advance now shows in its OWN column;"
echo "      OTHER holds only genuine other deductions. Columns appear"
echo "      dynamically per the firm's enabled deduction heads."
echo "   3. DUPLICATES: Repair Punches shows no same-time duplicates;"
