#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 580 — Sub-User Activity Monitoring + Audit Email Alerts)
# Deploys the FULL latest code (includes Iter 568-579). Running 580 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 580) ═══════════
#
# A. AUDIT EMAIL NOTIFICATION ENGINE (new screen: Administration →
#    Audit Notifications, Super Admin only):
#    * INSTANT email alerts for CRITICAL activities: employee deletion,
#      bank/salary/PF/ESIC/UAN field changes, payroll unlock,
#      permission/role/rights changes, challan modification, firm
#      access changes.
#    * Optional failed-login alert emails.
#    * DAILY HTML SUMMARY at 08:00 IST: totals, failed, critical,
#      per-user and per-module breakdown + "Send Yesterday's Summary
#      Now" test button.
#    * Configurable recipients + CC. Uses existing Resend email.
#
# B. USERS LOG REPORT UPGRADES:
#    * "Activity by User" rollup table: per sub-user Total / Employee /
#      Attendance / Payroll / Reports / Failed counts.
#    * New quick ranges: Today, Yesterday (plus 7/30/90 days).
#
# All audit records remain immutable & firm-scoped server-side.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy580.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy580.sh
...
echo -n "   Server badge is 580 (must say OK): "
grep -q 'APP_ITERATION = "580"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Audit notify engine — Iter 580 (must say OK): "
[ -f $APP_DIR/backend/routes/audit_notify.py ] && echo "OK" || echo "MISSING!"
echo -n "   Notifications UI — Iter 580 (must say OK): "
[ -f $APP_DIR/frontend/app/audit-notifications.tsx ] && echo "OK" || echo "MISSING!"
echo -n "   By-user rollup — Iter 580 (must say OK): "
grep -q 'Activity by User' $APP_DIR/frontend/app/users-log-report.tsx && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 580 deployed."
echo "  Setup: Administration → Audit Notifications → enable, add"
echo "  recipient emails, save. Test with 'Send Yesterday's Summary'."
echo "  Users Log Report now shows the Activity-by-User table."
