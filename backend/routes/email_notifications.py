...
async def compose_notification(payload: dict = Body(...),
    ...
    # User directive — "Mail ALL companies in a single click": super/sub
    # admins can broadcast to every firm at once.
    ...
    to_all = bool(payload.get("all_employees")) or all_companies
    ...
    if not all_companies and company_id:
    ...
    if send_inapp:
        docs = [{
            ...
            "company_id": t.get("company_id") if all_companies else company_id,
            "title": subject,
            "body": message,
