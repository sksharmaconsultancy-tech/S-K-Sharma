...
def _scope_company(user: Dict[str, Any], company_id: Optional[str]) -> str:
    if user["role"] in ("company_admin", "sub_admin"):
...
async def get_challan(
    ...
    if user["role"] in ("company_admin", "sub_admin") and doc.get("company_id") != user.get("company_id"):
...
async def download_portal_upload_file(
    ...
    if user["role"] in ("company_admin", "sub_admin") and job.get("company_id") != user.get("company_id"):
