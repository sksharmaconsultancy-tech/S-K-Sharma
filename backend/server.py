...
# Default (missing / empty list) → all features enabled (backward compat).
...
async def admin_set_employee_credentials(
    ...
    if admin["role"] in ("company_admin", "sub_admin"):
        if not admin.get("company_id") or target.get("company_id") != admin["company_id"]:
...
async def delete_company(company_id: str,
    ...
    cascade_report = {}
    # Iter 72.g — always sweep collections that store a `company_id`
    # reference, regardless of the `force` flag.  These are all
    # ancillary tables (audit trails, cached PDFs, session data, pending
    # signup requests, etc.) — never the source-of-truth employee list,
    # so it's safe to purge them on every delete.
    #
    # The `users` collection is treated separately because deleting
    # employees is the destructive operation that requires `force`.
    always_clean = (
        ...
        # New in Iter 72.g — pending signup / join / policy requests so a
        # deleted firm can't be reactivated by lingering approval rows.
        "company_requests", "company_documents", "bonus_runs",
        "compliance_salary_batches", "employee_policy", "branches",
        "company_audit_log",
        # Iter 75 — group policy templates
    ...
    if force:
        # Destructive step — wipe employee accounts too.
        try:
    ...
    for idx, r in enumerate(rows, start=1):
        try:
            ...
            doc: Dict[str, Any] = {
                ...
                "salary_mode": r.get("salary_mode") or "monthly",
                "salary_monthly": r.get("salary_monthly"),
                "compliance_gross": r.get("compliance_gross"),
...
async def admin_bulk_import_template(
    ...
    require_role(admin, ["company_admin", "super_admin", "sub_admin"])
    columns = [
        "name", "phone", "email", "father_name", "gender", "dob", "doj",
        "designation", "department", "employee_type",
        "salary_mode", "salary_monthly", "compliance_gross",
        "uan_no", "pf_no", "esi_ip_no", "pan_no", "aadhaar_no",
        "bank_name", "bank_account", "bank_ifsc", "address",
        # Iter 74 — pipe-separated Head:Amount lists, e.g.
        # actual_allowances = "HRA:2000|Convey:500|SpecialAllow:1000"
        # deductions = "Advance:500|Loan:2000"
        "actual_allowances", "actual_deductions",
        "compliance_allowances", "compliance_deductions",
    ]
    sample = [
        "Ramesh Kumar", "+919812345678", "ramesh@example.com", "Suresh Kumar",
        "Male", "1990-05-14", "2024-04-01",
        "Machine Operator", "Weaving", "Worker", "Shift-A",
        "monthly", "22000", "18000",
        "123456789012", "DL/PF/12345", "1122334455", "ABCDE1234F", "123412341234",
        "HDFC Bank", "1234567890", "HDFC0001234", "New Delhi",
        # Iter 74 — sample allowance / deduction rows.
        "HRA:2000|Convey:500|SpecialAllow:1000", "Advance:500",
        "HRA:1500|Convey:400", "PF:1800|ESI:135",
    ]
    body = (",".join(columns) + "\n" + ",".join(sample) + "\n").encode("utf-8")
    return Response(
        content=body,
...
# ---------------------------------------------------------------------------
# Employee Master PDF + Scan Documents (Iteration 50)
# ---------------------------------------------------------------------------
...
@api.get("/admin/employees/{user_id}/master-pdf")
...
@api.get("/admin/employees/master-pdf/bulk")
...
@api.get("/admin/employees/{user_id}/documents")
...
@api.post("/admin/employees/{user_id}/documents")
...
@api.get("/admin/employees/{user_id}/documents/{doc_id}")
...
@api.delete("/admin/employees/{user_id}/documents/{doc_id}")
