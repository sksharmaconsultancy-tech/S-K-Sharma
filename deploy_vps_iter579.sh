#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 579 — OTP Email Signature Styling)
# Deploys the FULL latest code (includes Iter 568-576). Running 577 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 577) ═══════════
#
# OTP EMAIL REBRANDED (per your request):
#   * Signature styled (Iter 579): navy footer band, "From S.K. Sharma
#     & Co" in BOLD GOLD, "Your Trusted Compliance Partner" bold italic
#     white — matching the brand header. Demo approved by user.
#   * "Sent at" date/time line REMOVED from OTP emails (Iter 578).
#   * Subject/heading: "Your Smart Payroll Login Code"
#   * Signature at the end:
#       From S.K. Sharma & Co
#       Your Trusted Compliance Partner
#   * Applied to both Resend and SMTP OTP emails.
#
# Also includes Iter 576 MSG91 SMS & OTP Phase 1.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy579.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy579.sh
...
echo -n "   Server badge is 579 (must say OK): "
grep -q 'APP_ITERATION = "579"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Smart Payroll OTP branding — Iter 577 (must say OK): "
grep -q 'Your Smart Payroll Login Code' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   MSG91 Phase 1 incl. (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 577 deployed. OTP mails now say"
echo "  'Your Smart Payroll Login Code' (no Sent-at date/time) + signature."
