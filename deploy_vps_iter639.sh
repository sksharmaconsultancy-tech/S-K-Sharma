#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 639)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 639) ═══════════
#
# ✅ APPROVE BACKLOG — ONE CLICK (user request):
#  * Punch Approvals screen: when there are pending punches, a green
#    "Approve All Pending (N)" button appears. One click (with a
#    confirmation) approves EVERY pending punch in the selected firm's
#    scope so the Attendance Grid is instantly up to date.
#  * Each record gets the full audit trail: decided-by admin id,
#    decision time, reason "Bulk approve — pending backlog cleared".
#  * New endpoint POST /api/attendance/punches/approve-all-pending
#    (super/sub admins may scope by ?company_id=, firm admins are
#    always locked to their own firm).
#
# 🔍 ATTENDANCE GRID — READABILITY (VIEW/UI ONLY):
#  * Same treatment as the salary sheets: employee names 14px, day
#    cells ~20% wider (80px), IN/OUT times 12px green/red, hours and
#    summary columns 13px, taller rows (8px vertical padding), header
#    day numbers 13.5px. Sticky header/columns unchanged; zero logic
#    changes.
#
# 🧰 Iter 638 (also included) — Configure Batch toolbar fixes ·
#    637 — single-line toolbar · 636 — Actual grid + compact header.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy639.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy639.sh
...
echo -n "   Server badge is 639 (must say OK): "
grep -q 'APP_ITERATION = "639"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 639 deployed."
echo "  • NEW (639): APPROVE BACKLOG — Punch Approvals now has a green"
...
echo "  • NEW (639): ATTENDANCE GRID readability — names 14px, day"
