#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 542)
#
...
#
# B. PUNCH-SEQUENCE ATTENDANCE (user rule, Iter 542) — activates when
...
#
# E. ESIC LEAVE (Iter 542, user requests):
...
#
# F. DAILY IN/OUT & OT VERIFICATION (Iter 542, user requests):
...
#
# G. "DELETED FROM MACHINE ✓" TAG (Iter 542): when a machine confirms a
...
#   wget -O deploy542.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy542.sh
...
grep -q 'APP_ITERATION = "542"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
