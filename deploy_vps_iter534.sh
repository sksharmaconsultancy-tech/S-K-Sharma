#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 534)
#
# ═══════════════ WHAT'S NEW SINCE 532 (this deploy) ═══════════════
#
# A. MONTHLY PAYROLL REPORT (user request, Iter 534):
...
#   wget -O deploy534.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy534.sh
...
grep -q 'APP_ITERATION = "534"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
