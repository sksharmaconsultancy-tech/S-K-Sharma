#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 584 — Device Sync Engine: Manual Register/Delete + Auto Master-Sync LOCKED)
# Deploys the FULL latest code (includes ALL of Iter 568-583). Running 584 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 584) ═══════════
#
...
#
# WHAT'S NEW vs 583 — DEVICE SYNC ENGINE FINAL RULES:
...
#   wget -O deploy584.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy584.sh
...
echo -n "   Server badge is 584 (must say OK): "
grep -q 'APP_ITERATION = "584"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
