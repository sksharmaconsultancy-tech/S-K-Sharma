#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 588 — 🤖 AI COMMAND CENTER)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 588) ═══════════
#
# 🤖 AI COMMAND CENTER — new sidebar item (bottom, above AI Payroll
#    Assistant). The central AI workspace with FIVE sections:
#
#  * ASK AI — full conversational assistant (English / Hindi / Hinglish),
#    same trusted engine as the floating assistant. Quick-command chips
#    (Employee count, Today's attendance, Salary run status, Missing bank
#    details, Generate salary register / bank sheet / PF ECR, …).
#    Actions still require explicit CONFIRM buttons; if a sensitive change
#    is made by a sub-admin, it is automatically routed to the
#    Maker-Checker approval queue instead of applying directly.
#  * APPROVALS — the Maker-Checker queue inline, now with AI RISK levels
#    on every request (Salary change = HIGH, Bank change = HIGH, Employee
#    deletion = CRITICAL). Approve / Reject / View Details.
#  * ALERTS — rule-based alert engine with 🔴 CRITICAL / 🟠 WARNING /
#    🔵 INFO severity: missing Aadhaar / bank / IFSC / UAN / ESIC,
#    duplicate bank accounts & UANs, active employees without salary
#    structure, salary changed >30% (>100% = critical) in last 31 days,
#    HELD/BLOCKED punches, pending approval backlog. Each alert links to
#    the screen where you fix it.
#  * INSIGHTS — automatic KPIs: employees (active/total/joiners/exits,
#    department-wise), payroll month totals (gross/net/PF/ESIC) with
#    month-over-month growth %, attendance (present today/%), compliance
#    (PF/ESIC numbers present, missing Aadhaar). This Month / Previous
#    Month period switch.
#  * ACTIVITY — immutable AI audit trail: every AI command is now written
#    to the same Users-Log architecture (AI_COMMAND rows with command,
#    interpretation, firm) alongside approval decisions.
#
#  SECURITY: every tab is server-side scoped through the central authz
#  engine — a sub-admin restricted to Firm A can NEVER see Firm B data in
#  any AI answer, alert, insight or activity row (verified by tests:
#  cross-firm requests return 403).
#
#  The floating AI assistant button keeps working on all other screens;
#  it hides only inside the Command Center itself. The existing AI
#  Payroll Assistant / AI Insights / AI Salary Compliance / AI Universal
#  Import screens are untouched and still available.
#
#  Tests: 12/12 backend + 7/7 UI end-to-end + full maker-checker
...
#   wget -O deploy588.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy588.sh
...
echo -n "   Server badge is 588 (must say OK): "
grep -q 'APP_ITERATION = "588"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   AI Command Center backend — Iter 588 (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 588 deployed."
echo "  Open the sidebar → 🤖 AI Command Center (bottom)."
