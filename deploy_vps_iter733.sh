#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 733)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 733) — 🏢 BRANCH P&L + 🗺️ STATE-WISE STATUTORY ═══════════
#
...
#   wget -O deploy733.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy733.sh
...
grep -q 'APP_ITERATION = "733"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
