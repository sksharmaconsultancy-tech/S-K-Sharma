#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 636)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 636) ═══════════
#
# 🖥  ACTUAL SALARY PROCESS GRID — same readability redesign (UI only):
#  * Grid text 11px → 14px, headers 14px bold, rows 32px → ~44px,
#    editable cells enlarged, auto-fit column widths recalibrated,
#    grid uses the full viewport height (all employees on one page,
#    sticky header/columns kept). Zero logic changes.
#
# 🗜  COMPLIANCE SALARY — COMPACT HEADER (user request):
#  * When a salary run is on screen, the "Select firm" and "Configure
#    batch" cards auto-collapse into ONE slim bar:
#      Firm · Month · Group        [ Change firm / month ▾ ]
#    so the salary grid starts right at the top of the page.
#  * Tap the bar to expand the setup cards again; a "Collapse setup —
#    jump to grid" link collapses them back. No workflow changes.
#
# 🖥  Iter 635 (also included) — compliance grid readability ·
#    634 — 1-min autosave + multi-tab fix · 633 — whole-rupee engine.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy636.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy636.sh
...
echo -n "   Server badge is 636 (must say OK): "
grep -q 'APP_ITERATION = "636"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 636 deployed."
echo "  • NEW (636): ACTUAL SALARY grid got the same readability"
echo "    upgrade — 14px text, 14px bold headers, ~44px rows, larger"
echo "    edit cells, full viewport height, all employees one page."
echo "  • NEW (636): COMPACT HEADER — once a compliance run is on"
