#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 635)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 635) ═══════════
#
# 🖥  COMPLIANCE SALARY GRID — READABILITY REDESIGN (VIEW/UI ONLY):
#  * Grid text 11px → 14px; column headers 14px bold; group bands
#    (Master / Calculated / Deductions) 13px.
#  * Row height increased to ~44px with proper vertical alignment.
#  * Auto-fit column widths recalibrated for the larger font; monetary
#    values stay right-aligned; Name / codes stay frozen on the left.
#  * The grid now uses the FULL available browser height
#    (calc(100vh − 170px)) instead of a fixed 640px window — ALL
#    active employees remain on ONE page (no pagination) with the
#    header row and identity columns sticky while scrolling.
#  * Editable cells (Present Days / OT Hrs / ESIC Leave …) grew to
#    match. STRICTLY NO changes to logic, calculations, APIs, buttons,
#    workflow or data — CSS/typography/layout only.
#
# 💾 Iter 634 (also included) — 1-minute autosave + multi-tab refresh
#    fix · 633 — whole-rupee engine + Excel (Displayed) · 632 —
#    whole-rupee exports · 631 — allowance disable warning.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy635.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy635.sh
...
echo -n "   Server badge is 635 (must say OK): "
grep -q 'APP_ITERATION = "635"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 635 deployed."
echo "  • NEW (635): COMPLIANCE GRID READABILITY (VIEW-ONLY redesign)"
