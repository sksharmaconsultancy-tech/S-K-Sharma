#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 567 — includes 555→566)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 567) ═══════════
#
# A. 24Q RECONCILIATION — EXCEL EXPORT [Iter 567 NEW]:
#    "Excel" button inside the TDS Reconciliation section downloads the
#    full comparison: per-quarter Payroll / Filed / Diff / Status per
#    employee + totals; MISMATCH rows highlighted red.
#
# B. "MY FORM 16" IN EMPLOYEE MENU [Iter 567 NEW]:
#    Employees now find "My Form 16" in their Profile tab menu (next to
#    My ID Card) — no URL typing needed.
#
# C. Earlier: legacy salary fallback + Form 16 Phase 2 [566], duplicate
#    name import guard [565], API URL banner [564], security alerts
#    [563], Test Console [562], Punching Push API [561], Performance-
#    sheet import [560], Punch Approvals Excel + Bio Code [559/558],
#    Time Format [557], ✎ badge [556], OT fix + Super Admins [555].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy567.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy567.sh
...
echo -n "   Server badge is 567 (must say OK): "
grep -q 'APP_ITERATION = "567"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Reconciliation Excel endpoint (must say OK): "
grep -q 'reconciliation_xlsx' $APP_DIR/backend/routes/form16.py && echo "OK" || echo "MISSING!"
echo -n "   Excel button in recon UI (must say OK): "
grep -q 'f16-recon-xlsx' $APP_DIR/frontend/app/form16.tsx && echo "OK" || echo "MISSING!"
echo -n "   My Form 16 menu row (must say OK): "
grep -q 'row-my-form16' "$APP_DIR/frontend/app/(tabs)/profile.tsx" && echo "OK" || echo "MISSING!"
echo -n "   Form16 Phase 2 — Iter 566 (must say OK): "
grep -q 'tds_reconciliation' $APP_DIR/backend/routes/form16.py && echo "OK" || echo "MISSING!"
echo -n "   Legacy salary fallback — Iter 566 (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 567 deployed."
echo "  Test 1: Form 16 → 24Q Reconciliation → 'Excel' downloads the"
echo "    full comparison (mismatches highlighted red)."
echo "  Test 2: employee login → Profile tab → 'My Form 16'."
