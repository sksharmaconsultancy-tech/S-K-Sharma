#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 649)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 649) ═══════════
#
# ⌨️ ROW HIGHLIGHT + ARROW-KEY NAVIGATION (user request):
#  * Compliance Salary AND Actual Salary grids: click any employee row
#    to select it; ↑ / ↓ arrow keys move the highlight to the previous /
#    next row (clamped at the first & last row) and auto-scroll it into
#    view. Never fires while typing in an editable cell; never refreshes
#    the page or clears the run. Pure UI — no calculation changes.
#
# 🧰 Iter 648-644 (also included) — Bulk Correction Rate/Pay Basis fix ·
#    Salary Lock hardened + editable INCENTIVE column · Import-sheet
#    FOOD ALLOWANCE fix · INCENTIVE columns in PDFs/grid/exports ·
#    OT toggle columns · 643 earlier fixes.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy649.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy649.sh
...
echo -n "   Server badge is 649 (must say OK): "
grep -q 'APP_ITERATION = "649"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 649 deployed."
echo "  • NEW (649): Row highlight + Up/Down arrow-key navigation on"
echo "    the Compliance & Actual salary grids (auto scroll-into-view)."
