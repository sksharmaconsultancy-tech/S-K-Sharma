#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 642)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 642) ═══════════
#
# 🧰 CONFIGURE BATCH — single-line filter alignment (user feedback):
#  * "Month (FY-wise)", "Month Days", "Employee Group" and the three
#    summary cards (Total / Processed / Pending) now all sit on ONE
#    horizontal line — no stacking or overlap. VIEW-ONLY, no logic
#    changes.
#
# 🧰 Iter 641 (also included) — Salary Process two-line compact button
#    · 640 — Month Days rename + narrow input, two-line Copy button
#    · 639 — Approve Backlog + Attendance Grid fonts · 638/637 —
#    Configure Batch toolbar · 636-635 — grid redesigns · 634 —
#    autosave + multi-tab fix.
#
...
echo -n "   Server badge is 642 (must say OK): "
grep -q 'APP_ITERATION = "642"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 642 deployed."
echo "  • NEW (642): Configure Batch filters — Month (FY-wise), Month"
