#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 637)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 637) ═══════════
#
# 🧰 CONFIGURE BATCH — COMPACT SINGLE-LINE TOOLBAR (VIEW/UI ONLY):
#  * "Configure Batch" title 22px bold with a gear icon.
#  * ONE horizontal line on desktop: Month (FY-wise) → Month Days
#    (override) → Employee Group → summary cards → actions; wraps
#    gracefully on narrow screens.
#  * Display-only summary cards on the same line: Total Employees
#    (blue), Processed ✓ (green), Pending ◷ (orange) — driven by the
#    existing group counts and the run already on screen (no new
#    API calls, no new calculations).
#  * Salary Process button now GREEN, Copy Last Month Salary purple —
#    ~48px tall, rounded 10px, 14-15px labels. Exact same actions.
#  * "Already processed with N days…" message moved below the toolbar
#    into a subtle light-blue info panel (same confirmation flow).
#  * UPPERCASE 12.5px bold field labels, 14px controls (48px tall).
#  * STRICTLY no changes to processing, validations, workflow or data.
#
# 🖥  Iter 636 (also included) — Actual grid redesign + compact header ·
#    635 — compliance grid readability · 634 — autosave + tab fix.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy637.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy637.sh
...
echo -n "   Server badge is 637 (must say OK): "
grep -q 'APP_ITERATION = "637"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 637 deployed."
echo "  • NEW (637): CONFIGURE BATCH single-line toolbar (VIEW-ONLY)"
