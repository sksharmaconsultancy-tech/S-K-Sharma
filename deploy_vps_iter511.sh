#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 511) — EMPLOYER PWA BLANK-PAGE FIX
#
# WHY 511: you reported the EMPLOYER PWA shows a BLANK PAGE. The code is
# healthy (verified in the workspace: employer login admin@kankani.local,
# home/tasks/documents/profile tabs AND the /employer install page all
# render fine in dev + production builds). The blank page comes from a
# STALE PWA SHELL on the phone: the installed app's service worker cached
# an old HTML that points at a JS bundle which no longer exists after a
# deploy — so nothing loads.
#
# PERMANENT FIX SHIPPED IN THIS BUILD (Iter 511):
#   • BLANK-PAGE SELF-HEAL baked into the app shell (+html): if the app
#     fails to boot (bundle 404, or splash stuck > 15 s) it automatically
#     unregisters the service worker, purges ALL caches and reloads —
#     nobody ever has to clear caches manually again.
#   • Service-worker cache bumped to v7 — every device purges its stale
#     shell on next open.
#   • Plus everything from Iter 510: STEP-0 diagnostics, swap/disk
#     protection, backend-first restart, rollback-safe publish.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy511.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy511.sh
...
echo -n "   Server badge is 511 (must say OK): "
grep -q 'APP_ITERATION = "511"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "   VERIFY: footer badge must read 'Server Iter 511'."
