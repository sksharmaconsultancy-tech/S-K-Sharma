#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 581 — Onboarding Gate: Attendance Eligibility)
...
#   wget -O deploy581.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy581.sh
...
echo -n "   Server badge is 581 (must say OK): "
grep -q 'APP_ITERATION = "581"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
