#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 582 — Onboarding Gate + Payroll Guard + Onboarding % widgets)
...
#   wget -O deploy582.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy582.sh
...
echo -n "   Server badge is 582 (must say OK): "
grep -q 'APP_ITERATION = "582"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
