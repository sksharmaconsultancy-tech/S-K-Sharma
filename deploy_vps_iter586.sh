#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 586 — RBAC: Scope wiring + Roles & Permissions UI + Sensitive Masking)
# Deploys the FULL latest code (includes ALL of Iter 568-585). Running 586 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 586) ═══════════
#
...
#
# WHAT'S NEW vs 585 — RBAC continued:
#  * DATA SCOPE now enforced on ATTENDANCE (day-status/punch approvals
#    source) and SALARY REGISTER (+ its Excel/CSV exports) — restricted
#    users see only their branch/department employees, server-side.
#  * ROLES & PERMISSIONS screen (Administration menu): pick any sub-admin /
#    client user -> full Module x View/Add/Edit/Delete/Export/Approve
#    matrix + Sensitive Data toggle + Branch/Department scope chips.
#    Every change audited (PERMISSION_CHANGED / DATA_SCOPE_CHANGED).
#  * SENSITIVE FIELD MASKING (Phase 2 core): Aadhaar/PAN/Bank/UAN/Mobile/
#    Address masked IN THE BACKEND RESPONSE unless the user has
#    sensitive_data:view. Unmasked access logs SENSITIVE_DATA_VIEWED
#    (field names only, deduped per day). One-time backward-compat
#    migration grants the permission to users who already had employee
#    access: POST /api/admin/access/migrate-sensitive-permission
#    (run once as super admin, then revoke per user as needed).
#  * 14/14 new tests + 32/32 Phase-1 regression tests pass.
#
# PREVIOUS — RBAC PHASE 1 (Iter 585):
...
#   wget -O deploy586.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy586.sh
...
echo -n "   Server badge is 586 (must say OK): "
grep -q 'APP_ITERATION = "586"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
