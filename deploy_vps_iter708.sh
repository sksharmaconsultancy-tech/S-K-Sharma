#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 708)
...
#   wget -O deploy708.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy708.sh
...
echo -n "   Server badge is 708 (must say OK): "
grep -q 'APP_ITERATION = "708"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q 'report.xlsx' $APP_DIR/backend/routes/tours.py && grep -q 'advance/settle' $APP_DIR/backend/routes/tours.py && [ -f $APP_DIR/backend/routes/my_approvals.py ] && [ -f $APP_DIR/frontend/app/my-approvals.tsx ] && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'employee-search' $APP_DIR/backend/routes/approvals_engine.py && grep -q 'leave-balance/bulk' $APP_DIR/backend/routes/leaves.py && grep -q 'leave-settings' $APP_DIR/backend/routes/leaves.py && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "28"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'typeof body === "string" ? body' $APP_DIR/frontend/src/api/client.ts && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'auto_captured' $APP_DIR/backend/routes/portal_extension.py && grep -q 'auto_captured' $APP_DIR/frontend/app/challans.tsx && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   ESIC challan record v28 + Smart Payroll rename — Iter 704 (must say OK): "
grep -q 'ESIC Challan Number' $APP_DIR/backend/routes/portal_extension.py && grep -q 'RUNNER_VERSION = "28"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'Smart Payroll' $APP_DIR/frontend/public/manifest-employee.json && grep -q 'Smart Payroll' $APP_DIR/frontend/scripts/inject-pwa-html.js && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'esic_open' $APP_DIR/backend/routes/portal_extension.py && grep -q '_diagnose_esic_creds' $APP_DIR/backend/routes/portal_extension.py && grep -q 'esic_open' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   TRRN + challan PDF + ESIC nav v27 — Iter 701 (must say OK): "
grep -q 'portal-ext/trrn' $APP_DIR/backend/routes/portal_extension.py && grep -q 'challan-pdf' $APP_DIR/backend/routes/portal_extension.py && grep -q 'as-download-challan' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'RUNNER_VERSION = "28"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   In/Out report grid cache + debounce — Iter 702 (must say OK): "
grep -q '_GRID_CACHE' $APP_DIR/backend/routes/inout_ot_matrix.py && grep -q 'qDeb' $APP_DIR/frontend/app/inout-ot-matrix.tsx && grep -q 'sks-pwa-v24' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
