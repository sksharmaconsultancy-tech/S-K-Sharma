#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 353)
# ROOT-CAUSE FIX for wrong staff/employee salary structures (SUVIDHI etc.):
#   • The old DB keeps ONE EmployeeMaster row PER YEAR per employee, and
#     EmployeeSalaryStructureDtl rows are year-specific (keyed by LinkID).
#     The previous sync joined on EmpID, which MULTIPLIED and SUMMED every
#     year's HRA/Conv rows — that's why MAN SINGH MALOO showed HRA 19044
#     instead of 2000 and Gross 51153 instead of 30800.
#   • NEW: structure rows are fetched via the year-exact LinkID join, so
#     each employee gets ONLY their CURRENT year's structure. Fallback
#     paths de-duplicate heads instead of summing them.
#   • EMPLOYEE CODE CORRECTION (user: "Employee Code Is Also Mismatch"):
#     employees matched by NAME with a different old-DB code get their
#     portal code corrected (e.g. SANJAY KUMAR LODHA 380 → 372) unless the
#     code is already taken. Sync shows a "codes_corrected" counter.
#   • REMOVED the bogus WP_Emp_Attendance "staff table" sync from Iter 351
#     (that table is a scratch export with 10 stale rows of another firm).
#   • NEW /api/legacy-query diagnostic endpoint (token-guarded, SELECT-only)
#     so the developer can verify old-DB data remotely without redeploys.
...
echo -n "   Year-safe structure join (must be > 0): "
grep -c "_firm_structures" $APP_DIR/backend/routes/legacy_import.py || true
echo -n "   Code correction (must be > 0): "
grep -c "codes_corrected" $APP_DIR/backend/routes/legacy_import.py || true
echo ""
echo "✅ Deploy Iter 353 complete! HARD-REFRESH the browser (Ctrl+Shift+R)."
