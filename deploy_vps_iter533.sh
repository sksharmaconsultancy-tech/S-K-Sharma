#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 533)
#
# ═══════════════════ WHAT'S NEW SINCE 532 ═══════════════════
#
# USER MANUAL updates (user request):
# 1. TWO NEW SECTIONS with real screenshots —
#    • "4. Add New Firm (Firm Master Setup)" — the 16-step Firm
#      Master editor (General Information etc.).
#    • "6. Add New Employee" — the Add New Employee form with
#      OCR scan buttons and completion tracker.
#    All later sections renumbered (manual is now 25 pages).
# 2. "Software Version" line REMOVED from both covers — the cover
#    now shows only Product By + Prepared By (Ankit Sharma).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy533.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy533.sh
...
grep -q 'APP_ITERATION = "533"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
