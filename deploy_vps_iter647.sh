#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 647)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 647) ═══════════
#
# 🔒 SALARY LOCK — HARDENED (user: "still not working"):
#  * The lock endpoint's PF/ESIC validator and audit writer can no longer
#    block the lock even if they crash on unusual data (Iter 423b
#    non-blocking policy fully enforced). Duplicate-UAN/IP detection now
#    also reads the live Employee Master IDs.
#
# ✏️ INCENTIVE COLUMN — EDITABLE (user request):
#  * Custom allowance head cells (INCENTIVE / FOOD ALLOWANCES / …) are
#    now editable on the salary grid; gross / PF / ESIC / net refresh
#    exactly like an Others* edit and the typed figure survives a
#    reprocess (kept via manual_fields).
#
# 🧰 Iter 646-644 (also included) — Import-sheet FOOD ALLOWANCE fix ·
#    INCENTIVE columns in PDFs/grid/exports · Lock popup fix · OT toggle
#    columns · 643 — PF ÷days import fix, frozen headers, 12px grid.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy647.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy647.sh
...
echo -n "   Server badge is 647 (must say OK): "
grep -q 'APP_ITERATION = "647"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 647 deployed."
echo "  • NEW (647): Salary Lock hardened — validator can never block"
echo "    the lock; INCENTIVE column is now editable on the grid."
