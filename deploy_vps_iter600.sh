#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 600 — MACHINE FACE-PHOTO SYNC FIX)
# Deploys the FULL latest code (includes ALL of Iter 568-599). Running 600 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 600) ═══════════
#
# 📷 BIOMETRIC MACHINE PHOTO SYNC — BUG FIXED:
#  * Root cause: "Sync FP/Face" pushed only fingerprints / face TEMPLATES
#      and user names to the target machine — the registered face PHOTOS
#      (USERPIC / BIOPHOTO) captured from machine 1 were never included,
#      so photos "did not sync" to machine 2.
#  * Now "Sync FP/Face" ALSO pushes every stored face photo (correct wire
#      per model: USERPIC for legacy, BIOPHOTO Type=9 for newer face
#      machines), skipping photos originally captured FROM that machine.
#  * "Fetch FP/Face" now also queries USERPIC + BIOPHOTO from the machine
#      (previously photos were only pulled once automatically).
#  * The result message shows "<templates> template(s) + <photos> face
#      photo(s) for <N> employee(s) queued".
#
# HOW TO SYNC PHOTOS MACHINE 1 → MACHINE 2:
#   1) Biometric Devices → machine 1 → "Fetch FP/Face" (wait ~1 min online)
#   2) Biometric Devices → machine 2 → "Sync FP/Face"
#   Note: machines have photo size limits (10-20 KB per photo depending on
#   model); oversized photos are rejected by the machine itself.
#
...
echo -n "   Server badge is 600 (must say OK): "
grep -q 'APP_ITERATION = "600"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
