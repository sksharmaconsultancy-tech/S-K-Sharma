#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 545)
#
# ═══════════ WHAT'S NEW SINCE 544 (this deploy) ═══════════
#
# A. CONFIGURABLE MULTIPLE PUNCH & MAXIMUM PUNCH POLICY (user spec):
#    Attendance Policy → Policy Master now has an ATTENDANCE PUNCH
#    POLICY section:
#      • Multiple Punch Allowed (existing switch — NOW FUNCTIONAL:
#        "No" = exactly one IN → OUT cycle / 2 punches per day)
#      • Maximum Punches Per Day (default 4, min 2, max 20 — never
#        hard-coded; chips 2/4/6/8/10 + free numeric input)
#      • Punch Sequence: IN → OUT → IN → OUT (validated)
#      • Extra Punch Action: Reject (default) / Mark as Exception
#      • Invalid Sequence Action: Reject (default) / Exception
#    ENFORCEMENT (backend, all punch paths):
#      • Mobile/PWA app punch — a punch beyond the limit is REJECTED
#        with "Punch Limit Reached — maximum N punches allowed for
#        today"; double IN / OUT-without-IN rejected with "Invalid
#        punch sequence…". Every rejection is recorded in the Punch
#        Exception Log. "Mark as Exception" additionally stores the
#        attempt as a visible non-counted EXCEPTION punch.
#      • Biometric machine punches — NEVER dropped: an over-limit
#        machine punch is stored with status="exception" (visible in
#        the punch log + Exception Report, excluded from duty/OT).
#      • Manual admin repair punches bypass the limit (audit-logged
#        correction workflow, unchanged).
#    BACKWARD COMPATIBLE: firms that never saved the new fields keep
#    UNLIMITED punches (exact legacy behaviour) until the Attendance
#    Policy is saved once. Historical attendance is NEVER recalculated;
#    duty/OT/salary/compliance engines are untouched.
#
# B. NEW REPORT — Reports → "Multiple Punch Report" (user request):
#    • Punch Register tab: per employee-day — every punch chip,
#      IN→OUT pairs, Duty / Break / OT hours and a "Punches: n / max"
#      pill (red when the limit is reached). Filter: multi-punch days
#      only ↔ all punched days; month + employee search.
#    • Exceptions tab: the Punch Exception Log — type (Max Punch Limit
#      Exceeded / Duplicate IN / Duplicate OUT / OUT-without-IN /
#      Multiple Punch Not Allowed), reason, policy applied, punch
#      count, source/device and time.
#
#    HOW TO CHECK AFTER DEPLOY:
#    1. Attendance Policy → Policy Master → set Maximum Punches = 4 →
#       Save. From the employee app punch IN/OUT 4 times → the 5th
#       punch must show "Punch Limit Reached… maximum 4 punches".
#    2. Reports → Multiple Punch Report → the day shows all punches,
#       Duty/Break/OT and "Punches: 4 / 4"; the rejected 5th attempt
#       appears in the Exceptions tab.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy545.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy545.sh
...
echo -n "   Server badge is 545 (must say OK): "
grep -q 'APP_ITERATION = "545"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo "1) Attendance Policy → Policy Master → ATTENDANCE PUNCH POLICY —"
echo "   set Maximum Punches Per Day, Extra Punch Action → Save."
echo "2) Reports → Multiple Punch Report — punch register + exceptions."
echo "3) OT punch fix (Iter 544): Grid → day → Fix IN + OUT Together →"
echo "   IN 09:00, OUT 18:00, OT Out 20:00 → Save → 4 punches + OT."
