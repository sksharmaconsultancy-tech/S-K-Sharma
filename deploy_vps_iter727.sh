#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 727)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 727) — 🌙 DAY DUTY + रात की OT: OUT PUNCH अब सही दिन पर ═══════════
#
...
#   wget -O deploy727.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy727.sh
...
grep -q 'APP_ITERATION = "727"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
