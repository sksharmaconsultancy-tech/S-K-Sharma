#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 656)
...
#   wget -O deploy656.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy656.sh
...
echo "==> 8b/9 Nginx hardening for BIG salary sheets (Iter 656)..."
...
# finalize/save requests with 504. Ensure generous limits on every
# enabled site config that proxies /api.
for CONF in /etc/nginx/sites-enabled/* /etc/nginx/conf.d/*.conf; do
  ...
  if ! grep -q "client_max_body_size" "$CONF"; then
    ...
    echo "   Patched $CONF (50m body / 300s timeouts) ✅"
  else
    sudo sed -i 's/client_max_body_size[[:space:]]*[0-9]*[km]\?;/client_max_body_size 50m;/' "$CONF"
    grep -q "proxy_read_timeout" "$CONF" || sudo sed -i '0,/client_max_body_size 50m;/s//client_max_body_size 50m;\n    proxy_read_timeout 300s;\n    proxy_send_timeout 300s;/' "$CONF"
    echo "   Updated $CONF (body size -> 50m) ✅"
  fi
done
...
echo "==> 8c/9 SPEED-UP: nginx compression, caching & HTTP/2 (Iter 656)..."
...
# S.K. Sharma & Co. — performance tuning (deploy Iter 656)
gzip on;
gzip_comp_level 5;
...
echo -n "   Server badge is 656 (must say OK): "
grep -q 'APP_ITERATION = "656"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 656 deployed."
echo "  • NEW (656): Workspace tab fix — switching tabs no longer wipes"
