...
# 10. PDF heading: FIRM NAME printed BIG + address + Date (single day
#     shows "Date: …", not "Period: X to X") — repeated on EVERY page.
# 11. "Generated: …" moved to the page bottom; "Generated by" and
#     "Verify:" text removed from the print (QR still carries the
#     verification code).
# 12. Table heading row is plain WHITE (blue background removed) in
#     both PDF and Excel.
# 13. Department band + SUBTOTAL/GRAND TOTAL rows: label cells MERGED
#     into one (band = whole row) in PDF and Excel.
