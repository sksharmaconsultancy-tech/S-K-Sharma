#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 624)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 624) ═══════════
#
# 🏬 MULTI-BRANCH ARCHITECTURE (user 20-point spec):
...
#   wget -O deploy624.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy624.sh
...
echo -n "   Server badge is 624 (must say OK): "
grep -q 'APP_ITERATION = "624"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
