#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 659)
...
#   wget -O deploy659.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy659.sh
...
echo "==> 8b/9 Nginx hardening for BIG salary sheets (Iter 659)..."
...
echo "==> 8c/9 SPEED-UP: nginx compression, caching & HTTP/2 (Iter 659)..."
...
# S.K. Sharma & Co. — performance tuning (deploy Iter 659)
gzip on;
gzip_comp_level 5;
...
echo -n "   Server badge is 659 (must say OK): "
grep -q 'APP_ITERATION = "659"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
