#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 605 — EXPENSE CLAIMS MODULE COMPLETE: Phases 2-4 (Employee UI, Approvals, Payments, Reports))
# Deploys the FULL latest code (includes ALL of Iter 568-604). Running 604 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 605) ═══════════
#
...
#   wget -O deploy605.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy605.sh
...
echo -n "   Server badge is 605 (must say OK): "
grep -q 'APP_ITERATION = "605"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 605 deployed. EXPENSE CLAIMS MODULE IS COMPLETE:"
