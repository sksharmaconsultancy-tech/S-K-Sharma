#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 510) — RECOVERY DEPLOY
#
# WHY 510: you reported "Not able to Open PWA and Web Portal". The code
# itself is healthy (verified end-to-end in the workspace: Super Admin +
# Sub Admin login, Web Portal and PWA all open fine on the Iter 509 code,
# in BOTH dev and production builds). So the problem is on the VPS —
# most likely one of:
#   a) the Iter 509 build step ran out of memory / disk and aborted,
#      leaving a broken half-deployed web folder,
#   b) the backend service failed to restart (portal loads but hangs), or
#   c) the phone PWA is stuck on a stale cached shell.
#
# THIS SCRIPT IS SELF-DIAGNOSING AND SELF-REPAIRING:
#   • Prints full diagnostics FIRST (disk, memory, backend status + logs,
#     nginx) — if anything still fails, SEND ME THE FULL OUTPUT.
#   • Frees disk space (old caches) and adds swap if RAM is low so the
#     web build cannot be OOM-killed again.
#   • Restarts + health-checks the BACKEND BEFORE the long web build, so
#     the portal comes back up immediately.
#   • Backs up the current web folder and only replaces it AFTER the new
#     build succeeds — the portal can never go dark from a failed build.
#   • Bumps the PWA service-worker cache to v6 — every phone/browser
#     purges its stale shell on next open.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy510.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy510.sh
...
echo -n "   Server badge is 510 (must say OK): "
grep -q 'APP_ITERATION = "510"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   PWA cache bumped to v6 (must say OK): "
grep -q 'sks-pwa-v6' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
echo ""
echo "   VERIFY: footer badge must read 'Server Iter 510'."
