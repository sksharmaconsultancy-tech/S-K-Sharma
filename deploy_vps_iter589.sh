#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 589 — AI BULK-ACTION ENGINE)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 589) ═══════════
#
# 🧮 AI BULK-ACTION ENGINE (spec §16 — bulk action protection):
#  * Say in the AI Command Center / assistant:
#      "Increase salary of all employees in Production department by 5%"
#      "Sabki salary 500 rupaye badhao"  (firm-wide, flat amount)
#      "Reduce Packing dept salaries by 10%"
#  * The AI NEVER executes automatically. It shows a full PREVIEW first:
#      employees affected, current payroll, estimated new payroll,
#      difference, and per-employee old → new samples.
#  * SUPER ADMIN sees [Confirm & Execute]; sub-admins/client users see
#      [Send for Approval] — the change goes to the Maker-Checker queue as
#      CRITICAL risk and ONLY the Super Admin can approve it. Original
#      salaries stay untouched until approved.
#  * Every executed bulk change writes ONE salary_history row PER employee
#      (with bulk id + approval id) and a CRITICAL audit-log entry.
#  * Previews expire in 30 minutes; a preview can be executed only once;
#      authorization is re-checked at execute time; cross-firm requests
#      are rejected (403).
...
#   wget -O deploy589.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy589.sh
...
echo -n "   Server badge is 589 (must say OK): "
grep -q 'APP_ITERATION = "589"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 589 deployed."
echo "  Try a bulk command in the 🤖 AI Command Center:"
echo "    'Increase salary of all employees in <dept> by 5%'"
echo "  → preview → Confirm & Execute (or approval for sub-admins)."
