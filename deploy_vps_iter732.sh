#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 732)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 732) — 🧾 F&F: पूरा SETTLEMENT LIFECYCLE ═══════════
#
...
#   wget -O deploy732.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy732.sh
...
grep -q 'APP_ITERATION = "732"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
