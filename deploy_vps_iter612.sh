#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 612 — Firm Master gates relaxed + Iter 611 features)
...
#   wget -O deploy612.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy612.sh
...
echo -n "   Server badge is 612 (must say OK): "
grep -q 'APP_ITERATION = "612"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 612 deployed. EMPLOYEE SELF-SERVICE (ESS) IS LIVE:"
