#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 696)
...
#   wget -O deploy696.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy696.sh
...
echo -n "   Server badge is 696 (must say OK): "
grep -q 'APP_ITERATION = "696"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "23"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v14' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
