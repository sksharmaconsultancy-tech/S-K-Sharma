#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 646)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 646) ═══════════
#
# 🐛 IMPORT SHEET — FOOD ALLOWANCE / OT allocation FIX (user bug, BMD):
#  * Sheet amounts on custom allowance heads (FOOD ALLOWANCES / …) now
#    import PER-HEAD and land under their OWN column — they NEVER dump
#    into OT anymore. ESIC follows the corrected earnings.
#  * The freeze difference goes to OT only when the firm allows OT AND
#    the "OVER TIME" head is enabled in the catalog; otherwise it goes
#    to Other Allowances.
#
# 🧰 Iter 645-644 (also included) — INCENTIVE columns in PDF registers,
#    grid + Excel/CSV · Salary Lock fix · OT columns follow the OVER
#    TIME toggle · 643 — Excel-import PF ÷days fix, frozen headers,
#    12px grid font · 642-634 earlier fixes.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy646.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy646.sh
...
echo -n "   Server badge is 646 (must say OK): "
grep -q 'APP_ITERATION = "646"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 646 deployed."
echo "  • NEW (646): Import sheet FOOD ALLOWANCE lands under its own"
echo "    column — never in OT; OT allocation respects the OVER TIME"
echo "    toggle; ESIC follows the corrected earnings."
