#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 575 — SMTP Host Guard)
# Deploys the FULL latest code (includes Iter 568-574). Running 575 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 575) ═══════════
#
# SMTP TEST FAILED FIX ("Name or service not known"):
#   The SMTP Host field had an EMAIL ADDRESS in it. Host must be the
#   mail server name. Now:
#   * If you type a @gmail.com address in Host, the app auto-corrects
#     Host to smtp.gmail.com and moves the address to Username.
#   * Any other email in Host is rejected with a clear message.
#   * The Host label now says: "server name — NOT your email".
#
#   CORRECT GMAIL SMTP VALUES:
#     Host: smtp.gmail.com   Port: 587 (STARTTLS)
#     Username: sksharmaconsultancy@gmail.com
#     Password: your 16-character Google App Password (NOT your Gmail password)
#
# Also includes Iter 574: Resend custom-domain auto-fallback + SMTP
# auto-rescue for sub-user OTPs.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy575.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy575.sh
...
echo -n "   Server badge is 575 (must say OK): "
grep -q 'APP_ITERATION = "575"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   SMTP host guard — Iter 575 (must say OK): "
grep -q 'not an email address' $APP_DIR/backend/routes/email_notifications.py && echo "OK" || echo "MISSING!"
echo -n "   SMTP auto-rescue — Iter 574 incl. (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 575 deployed."
echo "  Fix your Email SMTP settings: Host=smtp.gmail.com, Port=587,"
echo "  Username=your Gmail, Password=Google APP PASSWORD → Send Test."
echo "  Then sub-user OTPs deliver via SMTP automatically."
