#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 541)
#
...
#
# B. PUNCH-SEQUENCE ATTENDANCE (user rule, Iter 541) — activates when
...
#
# E. ESIC LEAVE (Iter 541, user requests):
...
#
# F. DAILY IN/OUT & OT VERIFICATION (Iter 541, user requests):
...
#   wget -O deploy541.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy541.sh
...
grep -q 'APP_ITERATION = "541"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
