#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 640)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 640) ═══════════
#
# 🧰 CONFIGURE BATCH — FINAL POLISH (user feedback):
#  * Days field label renamed to just "MONTH DAYS" and the column
#    narrowed to fit its 2-digit input (functionality unchanged — the
#    max-days validation and the different-value confirmation still
#    work exactly the same).
#  * "Copy Last Month Salary" is now a compact TWO-LINE button.
#
# ✅ Iter 639 (also included) — Approve Backlog one-click + Attendance
#    Grid readability · 638/637 — Configure Batch toolbar · 636/635 —
#    salary grid redesigns · 634 — autosave + multi-tab fix.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy640.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy640.sh
...
echo -n "   Server badge is 640 (must say OK): "
grep -q 'APP_ITERATION = "640"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 640 deployed."
echo "  • NEW (640): Configure Batch polish — days field renamed to"
