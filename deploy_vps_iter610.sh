#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 610 — EMPLOYEE SELF-SERVICE (ESS) Phase 1)
# Deploys the FULL latest code (includes ALL of Iter 568-609). Running 610 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 610) ═══════════
#
...
#   wget -O deploy610.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy610.sh
...
echo -n "   Server badge is 610 (must say OK): "
grep -q 'APP_ITERATION = "610"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 610 deployed. EMPLOYEE SELF-SERVICE (ESS) IS LIVE:"
