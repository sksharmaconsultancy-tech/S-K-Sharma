#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 540)
#
...
#
# B. PUNCH-SEQUENCE ATTENDANCE (user rule, Iter 540) — activates when
...
#
# C. RULE BADGE (Iter 540): the Daily In/Out & OT Verification header
#    now shows the firm's calculation mode — green "Rule: Punch
#    Sequence" when the sub-point is ON, blue "Rule: Standard
#    (Shift HRS)" otherwise.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy540.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy540.sh
...
grep -q 'APP_ITERATION = "540"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
