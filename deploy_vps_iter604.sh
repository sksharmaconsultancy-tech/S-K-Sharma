#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 604 — EXPENSE CLAIMS MODULE Phase 1: backend engine + AI OCR)
# Deploys the FULL latest code (includes ALL of Iter 568-603). Running 604 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 604) ═══════════
#
# 💰 EXPENSE CLAIMS & REIMBURSEMENT — PHASE 1 (backend engine):
#  * Expense categories master — 33 defaults auto-seeded per firm
#      (Travel/Food/Accommodation/Office/Client/Other groups), admin can
#      add/edit/deactivate.
#  * Claims API: create draft (employee info snapshotted from Employee
#      Master), auto claim numbers (EC2608-0001…), submit → pending
#      manager, employee dashboard totals, claim history + audit trail.
#  * Receipts: PDF/JPG/PNG upload (5 MB cap, type-validated, stored in
#      MongoDB, auth-gated download — never public URLs).
#  * DUPLICATE DETECTION: same employee+date+amount or vendor+invoice →
#      409 "Potential Duplicate" requiring explicit confirmation.
#  * OFFLINE-READY: client_txn_id idempotency — the same offline claim
#      never syncs twice.
#  * 🤖 AI OCR (Emergent LLM key, GPT-5.4 vision): POST /api/expense/ocr
#      extracts vendor/invoice/date/total/GST from a receipt photo for the
#      employee to CONFIRM (never auto-approves). Lab test: hotel receipt
#      extracted 100% correctly.
#  ⏭ NEXT: employee/admin UI screens, approval workflow (Manager →
#      Accounts → Finance), policy engine, payments, payroll head,
#      reports (Phases 1b-4).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy604.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy604.sh
...
echo -n "   Server badge is 604 (must say OK): "
grep -q 'APP_ITERATION = "604"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
