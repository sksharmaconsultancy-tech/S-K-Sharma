#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 630)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 630) ═══════════
#
# ⚖️  ALLOWANCE HEAD ENABLE/DISABLE & FREEZE RECONCILIATION (user
#    7-rule spec) — the Firm-Master allowance mask now applies INSIDE
#    the salary engine:
#  * RULE 1 — disabled EDITABLE heads calculate as 0 on the next
#    Generate/Reprocess; Gross, ESIC & PT bases genuinely exclude them.
#  * RULE 2/3 — Freeze imports: Final Gross Paid == Imported Gross;
#    the masked amount joins the Difference and is reallocated ONLY
#    into OT / Other Allowances (the permitted adjustment heads) so
#    every column adds up (previously the masked amount silently
#    vanished from the visible columns while Gross stayed 20,000).
#  * RULE 4 — stored master / imported values are NEVER overwritten
#    (Stored Value → Configuration Mask → Calculated Value).
#  * RULE 5 — re-enabling changes nothing until the admin explicitly
#    Reprocesses; the saved value is then restored and statutory is
#    recalculated on the restored structure.
#  * RULE 6/7 — Basic and all fixed components are never masked or
#    redistributed; PF (PF-Basic driven) is untouched.
#  * Verified by an 18-assertion automated conformance test.
#
# ✅ Iter 629 (also included) — PWA punch time fix + auto-approve all
#    PWA punches · Iter 628 — Dummy Shift Matrix · Iter 627 — Shift
#    Deployment Summary Only.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy630.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy630.sh
...
echo -n "   Server badge is 630 (must say OK): "
grep -q 'APP_ITERATION = "630"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 630 deployed."
echo "  • NEW (630): ALLOWANCE ENABLE/DISABLE CONTRACT — disabling an"
