#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 573 — Sub-User OTP Email Routing)
# Deploys the FULL latest code (includes Iter 568-572). Running 573 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 573) ═══════════
#
# WHY ALL OTPs WERE LANDING ON sksharmaconsultancy@gmail.com:
#   Your RESEND email account is in TEST MODE (no verified domain), so
#   Resend refuses to email anyone except the account owner. The app
#   was forwarding sub-admin OTPs to your inbox so nobody got locked out.
#
# WHAT CHANGED (Iter 573):
#   * Forward-to-admin fallback is now OFF by default — your Gmail will
#     no longer receive other users' OTPs.
#   * NEW toggle in Administration → Security · 2FA/MFA:
#     "Send OTP via own SMTP (Email Settings)" — routes login OTPs
#     through your configured SMTP (e.g. Gmail App Password), which
#     delivers to EVERY sub user's own registered email.
#   * Clear on-screen message when Resend blocks delivery, with the fix.
#
# TO MAKE SUB-USER OTP WORK, PICK ONE:
#   Option A (recommended, 10 min): verify smartpayrolling.com at
#     resend.com/domains → set RESEND_FROM_EMAIL=no-reply@smartpayrolling.com
#     in backend/.env → restart backend. OTPs deliver to anyone.
#   Option B: configure SMTP in Email Settings (Gmail App Password) and
#     switch ON "Send OTP via own SMTP" in Security · 2FA/MFA.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy573.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy573.sh
...
echo -n "   Server badge is 573 (must say OK): "
grep -q 'APP_ITERATION = "573"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   SMTP OTP routing — Iter 573 (must say OK): "
grep -q 'otp_email_via_smtp' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Fallback now opt-in — Iter 573 (must say OK): "
grep -q '"fallback_to_admin_email": False' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "── Reset OTP routing to the new defaults in the DB ──"
...
db.security_settings.update_one({"key": "2fa"},
    {"$set": {"otp_email_via_smtp": False, "fallback_to_admin_email": False}}, upsert=True)
print("   Fallback to admin email: OFF ✓ (enable SMTP OTP in Security · 2FA/MFA if needed)")
PYEOF
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 573 deployed."
echo "  Your Gmail will NO LONGER receive sub users' OTPs."
echo "  To deliver OTPs to sub users, do Option A or B (see header)."
