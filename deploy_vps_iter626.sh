#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 626)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 626) ═══════════
#
# 👷 DAILY-RATED COMPLIANCE ENGINE ENHANCEMENTS (user 16-point spec):
#  * All existing daily logic PRESERVED (rate × payable days, per-day PF
#    Basic × days, full-month-equivalent ceilings, ESIC eligibility vs
#    contribution, adopted-higher-PF, wage-definition rule, head mapping).
#  * NEW: MID-MONTH DAILY RATE REVISION — Employee Master edit modal has a
#    "Daily Rate Revisions" box (one per line: YYYY-MM-DD:rate). Fresh
#    salary processes weight each rate period (by actual punch dates, or
#    calendar-proportionally for manual sheets) into ONE consolidated row;
#    historical months are never recalculated. Audit stored on the row.
#  * NEW: calc_detail on every salary row — transparent audit values:
#    rate, eligible paid days, OT rate/multiplier, PF Basic/day, PF
#    monthly equivalent vs earned vs contribution wage, PF ceiling, ESIC
#    coverage vs contribution wage, configured rates.
#  * Monthly employees: ZERO change (regression-tested: 14/14 + 6/6 PASS).
#
# 🏬 Iter 624-625 (also included) — MULTI-BRANCH ARCHITECTURE (user 20-point spec):
#  * Branch Master extensions: Branch Code (unique per firm), Address,
#    State/City/PIN, Branch Head, Email, Mobile, Active/Inactive + LINK an
#    existing branch of another firm to this firm.
#  * Employee Home Branch + Authorized Branches (Home NEVER auto-changes).
#  * Temporary Branch Assignments (from–to dates, reason, approver;
#    history preserved, cancel = status change only).
#  * Permanent Branch Transfers — effective-dated; historical records keep
#    the old branch; applies automatically on the effective date.
#  * PUNCH AUTHORIZATION: employees WITH a branch config can punch only at
#    Home/Authorized branches or under an approved temp assignment (403
#    otherwise). Employees WITHOUT branch config keep today's behaviour.
#    Every punch stores Home Branch vs Worked Branch.
#  * BRANCH COST ALLOCATION: ONE consolidated payroll per employee — cost
#    split by days worked per branch (Gross + Net + PF/ESIC liability
#    shares). One payslip, no duplicate PF/ESIC. Rahul test: ₹30,000, A 29
#    days + B 1 day → A ₹29,000 / B ₹1,000 ✓ (automated test).
#  * BRANCH DASHBOARD: per-branch employees, present today, worked/guest
#    days, salary cost, PF/ESIC liability, joiners/exits, cross-branch
#    count, month filter. Open: Branches screen → ⚙ → Dashboard.
#  * Branch RBAC hook: sub-admins with branch_admin_branch_ids see only
#    their branches on the new endpoints.
#
# 📄 Iter 623 (also included) — SALARY COMPLIANCE REPORT — FORMAT 2 FIX (user bug):
#  * UAN No. / EPF No. / ESIC No. columns no longer OVERWRITE the
#    neighbouring columns — long IDs now wrap inside their own column
#    (same fix Format 1 received earlier).
#
# 🔒 Iter 622 (also included) — PF & ESIC PRORATION — LOCKED TO MONTH DAYS (user decision, ALL firms):
...
#    PF will become 12% of the earned Wage Base (e.g. LAL CHAND 4333 → 520).
#
# 🏷️ Iter 621 (also included) — PF PRORATION BADGE:
#  * The salary sheet toolbar now shows an amber badge with the ACTIVE
#    PF Proration Method (e.g. "PF ÷26 (WORKING DAYS)" or
#    "PF ÷30 (CALENDAR DAYS)"), and the bottom totals strip repeats it —
#    admins instantly see which rule the PF column follows.
#  * Hidden automatically for firms where PF is disabled in Firm Master.
#
# 💰 Iter 620 (also included) — COMPLIANCE SALARY — 3 BUG FIXES (user report, firm SEO GROWTH):
#  1. "PF changed after Save + Reprocess": the on-screen grid was
#     calculating PF as present ÷ Month Days, while the server follows
#     the firm's Compliance Settings "PF Proration Method" (this firm:
#     Working Days = ÷26). The grid now uses the SAME configured method,
#     so PF no longer flips between typing, Save and Reprocess.
#     ➜ If you want PF = 12% of the earned wage base on a 30-day sheet,
#       set PF Proration Method = "Calendar Days" in Compliance Settings
#       and reprocess.
#  2. "Wage Base not showing on first-time process": typing Present Days
#     into a fresh sheet now rebuilds the Wage Base column
#     (max(Basic, floor% of Gross)) instead of leaving it 0.
#  3. After DELETING a salary run, the "sheet already exists → Reprocess?"
#     dialog no longer appears — the runs list refreshes on delete.
#
# ⌨️ Iter 619 (also included) — KEYBOARD SHORTCUTS PHASE 3:
#  * Employee Master: Alt+N new employee, Ctrl+F focus search,
#      Ctrl+Shift+E bulk Master PDF, Ctrl+S saves the quick-edit modal.
#  * Employee Form: Ctrl+S saves the draft (works while typing).
#  * Advances: Alt+N opens the New Advance form.
#  * Claims: Alt+N jumps to the New Claim tab.
#  * CUSTOM KEYS: press ? → click the ✎ next to any shortcut and press
#      your own key combo (saved per device); conflicts are rejected with
#      a clear message; "Reset to defaults" restores everything.
#  * Cleanup: the old duplicate shortcuts popup was merged into the one
#      searchable help overlay (g-then-key quick nav still works).
#
# 💰 Iter 618 (also included) — COMPLIANCE SALARY GRID KEYBOARD FIX:
...
#   wget -O deploy626.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy626.sh
...
echo -n "   Server badge is 626 (must say OK): "
grep -q 'APP_ITERATION = "626"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
