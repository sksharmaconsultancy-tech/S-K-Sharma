#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 553)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 551 + 552) ═══════════
#
# A. FORM 16 MODULE — PHASE 1 (Iter 551):
#    Payroll → Salary Process → "TDS · Form 16": FY dashboard,
#    auto payroll consolidation Apr→Mar (Compliance runs, monthly
#    salary + TDS), configurable NEW-regime Tax Master (₹75,000 std
#    deduction, rebate 87A, 4% cess), Readiness Check (PAN critical),
#    EXTRA income/deduction heads at generation time (＋ button),
#    statutory A4 PDF + Bulk ZIP, versioned history + audit.
#
# B. TAN + PAN QUICK ENTRY (Iter 553, enables your workflow):
#    • Employer TAN missing → inline "Save TAN" box right on the
#      Form 16 screen (validated ABCD12345E).
#    • Employee PAN missing → inline PAN box on each employee row
#      with a green ✓ save (validated ABCDE1234F) — readiness turns
#      Ready instantly, no Employee Master round-trip needed.
#
# C. SOFTWARE FEATURES LIST PDF (Iter 551): User Manual screen →
#    amber "Software Features List (PDF)" button.
#
#    HOW TO USE AFTER DEPLOY:
#    1. Payroll → Salary Process → TDS · Form 16 → FY 2025-26
#    2. Save employer TAN in the amber box
#    3. Fill missing PANs inline (green ✓)
#    4. Select All Ready → Generate → download PDF / Bulk ZIP
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy553.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy553.sh
...
echo -n "   Server badge is 553 (must say OK): "
grep -q 'APP_ITERATION = "553"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
