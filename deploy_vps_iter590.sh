#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 590 — AI DIRECT NAVIGATION & DIRECT ACTIONS)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 590) ═══════════
#
# 🧭 AI DIRECT NAVIGATION & DIRECT ACTIONS (user spec):
#  * "Attendance kholo" / "Open Employee Master" / "PF report open karo" /
#      "take me to salary process" → the AI now OPENS THE PAGE IMMEDIATELY.
#      No more [Open …] buttons for clear navigation commands — works in
#      English, Hindi and Hinglish, in both the AI Command Center and the
#      floating assistant.
#  * CONTEXT + FILTERS: "Open August attendance for Kankani Enterprises" →
#      switches the active firm to Kankani AND opens the Attendance Report
#      with 2026-08 already selected. No re-selecting firm/month.
#  * PERMISSION-GATED: navigation is checked SERVER-SIDE first — a
#      sub-admin without the module's view permission gets "You don't have
#      permission to access <page>" and nothing opens. Firm Master stays
#      super/sub-admin only.
#  * AMBIGUOUS pages ask instead of guessing: "open salary" → "Which salary
#      page: Actual, Compliance, OT, Arrear or Salary Reports?"
#  * SAFE actions (report/bank-sheet/ECR downloads, data questions) keep
#      executing directly; SENSITIVE actions (salary/bank/status changes,
#      deletes, bulk updates) STILL require explicit Confirm / Maker-Checker
#      approval — nothing was relaxed there.
#  * Tests: 10/10 direct-navigation tests + bulk/maker-checker regressions.
#
...
#   wget -O deploy590.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy590.sh
...
echo -n "   Server badge is 590 (must say OK): "
grep -q 'APP_ITERATION = "590"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 590 deployed."
echo "  Try it: tell the AI 'attendance kholo' or"
echo "  'Open August attendance for Kankani Enterprises' — the page"
echo "  opens instantly with the firm & month already selected."
