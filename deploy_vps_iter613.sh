#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 613 — Conditional Attendance Policy rule + Iter 611-612 features)
# Deploys the FULL latest code (includes ALL of Iter 568-611). Running 612 alone is enough.
#
# ═══════════ WHAT'S NEW (Iter 612) ═══════════
# 🏢 FIRM MASTER — no forced data (user directive):
#  * Attendance Policy is now OPTIONAL (label + warning removed).
#  * Health checklist no longer penalises: company logo, bank details,
#      attendance policy, compliance-doc count. Firms are fully usable
#      without this data.
#
# (Also includes Iter 611: Self Face Enrollment w/ HR approval + second
#  Super Admin login seed — see below.)
#
...
#   wget -O deploy613.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy613.sh
...
echo -n "   Server badge is 613 (must say OK): "
grep -q 'APP_ITERATION = "613"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 613 deployed. EMPLOYEE SELF-SERVICE (ESS) IS LIVE:"
