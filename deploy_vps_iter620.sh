#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 620)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 620) ═══════════
#
# 💰 COMPLIANCE SALARY — 3 BUG FIXES (user report, firm SEO GROWTH):
...
#   wget -O deploy620.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy620.sh
...
echo -n "   Server badge is 620 (must say OK): "
grep -q 'APP_ITERATION = "620"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
