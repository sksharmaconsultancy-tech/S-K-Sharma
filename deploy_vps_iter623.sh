#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 623)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 623) ═══════════
#
# 📄 SALARY COMPLIANCE REPORT — FORMAT 2 FIX (user bug):
...
#   wget -O deploy623.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy623.sh
...
echo -n "   Server badge is 623 (must say OK): "
grep -q 'APP_ITERATION = "623"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
