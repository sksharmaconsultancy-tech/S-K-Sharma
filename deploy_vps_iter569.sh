#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 569 — 2FA/MFA Login Security)
# NOTE: this script deploys the FULL latest code, so it also includes
# Iter 568 (Detailed Audit Trail). Running 569 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 569) ═══════════
#
# A. 2FA/MFA LOGIN SECURITY — SUPER ADMIN & SUB SUPER ADMIN [Iter 569 NEW]:
#    * Backend-enforced: password/PIN → OTP verification → dashboard.
#      NO session token is issued until the OTP verifies server-side.
#    * 6-digit OTP: crypto-random, HASHED at rest, 5-min validity,
#      one-time use, max 5 attempts (then blocked), 30s resend cooldown.
#    * Channels: Email (LIVE via Resend), WhatsApp + SMS (built-in,
#      activate later by adding provider credentials in the new
#      Administration → Security · 2FA/MFA screen).
#    * New verification screen with masked email/mobile, countdown,
#      resend, and switch-method options.
#    * Security Settings screen (super admin): OTP rules, channel
#      toggles, WhatsApp/SMS provider config (secrets masked),
#      trusted-device policy (OFF by default), trusted device list
#      with revoke, "Logout from all devices".
#    * Full audit integration: OTP_GENERATED / OTP_SENT_EMAIL /
#      OTP_VERIFICATION_SUCCESS / _FAILED / OTP_BLOCKED / OTP_RESEND /
#      TRUSTED_DEVICE_* / ALL_SESSIONS_REVOKED — all visible in the
#      Users Log Report with IP + device + status.
#    * Firm admins & employees: login flow UNCHANGED (no 2FA forced).
#
# B. DETAILED AUDIT TRAIL (Iter 568 — included): field-level OLD → NEW
#    diffs, summary cards, quick filters, View Details modal, Excel+.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy569.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy569.sh
...
echo -n "   Server badge is 569 (must say OK): "
grep -q 'APP_ITERATION = "569"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   2FA routes — Iter 569 (must say OK): "
[ -f $APP_DIR/backend/routes/twofa.py ] && echo "OK" || echo "MISSING!"
echo -n "   2FA login gate — Iter 569 (must say OK): "
grep -q '_start_2fa_challenge' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   OTP verify screen — Iter 569 (must say OK): "
[ -f $APP_DIR/frontend/app/verify-2fa.tsx ] && echo "OK" || echo "MISSING!"
echo -n "   Security settings UI — Iter 569 (must say OK): "
[ -f $APP_DIR/frontend/app/security-2fa.tsx ] && echo "OK" || echo "MISSING!"
echo -n "   Audit trail (Iter 568 incl.) (must say OK): "
[ -f $APP_DIR/backend/shared/audit.py ] && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 569 deployed (includes Iter 568)."
echo "  Test 1: Log in as Super Admin — you will be asked for a"
echo "    6-digit OTP sent to your registered EMAIL before entering."
echo "  Test 2: Administration → Security · 2FA/MFA — review OTP"
echo "    rules; add WhatsApp/SMS provider credentials when ready."
echo "  Test 3: Reports → Users Log Report — see LOGIN OTP_* events"
echo "    and field-level Old → New diffs (tap any row)."
echo "  NOTE: Firm admin / employee logins are NOT affected."
