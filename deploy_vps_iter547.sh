#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 547)
#
# ═══════════ WHAT'S NEW SINCE 546 (this deploy) ═══════════
#
# A. MERGED IN/OUT + OT SHEET (user request):
#    "Show all 4 punches in a single IN/OUT sheet — right now both
#     sheets (IN/OUT and OT IN/OUT) are separate."
#
#    The Attendance Report → IN / OUT sheet now shows the OT punches
#    directly inside each day cell:
#        IN   08:00
#        OUT  20:00
#        OT 20:05–08:00 · 11:55   (amber line, only on OT days)
#        10:50                    (duty hrs)
#    All 4 punches in ONE sheet. The separate "OT IN / OUT" tab stays
#    unchanged for OT-focused review.
#
#    HOW TO CHECK AFTER DEPLOY: Attendance Report → IN / OUT →
#    VINIT LODI → 08 Aug cell must show IN/OUT + the amber OT line.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy547.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy547.sh
...
echo -n "   Server badge is 547 (must say OK): "
grep -q 'APP_ITERATION = "547"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q 'Iter 547' $APP_DIR/frontend/app/attendance-grid.tsx && echo "OK" || echo "MISSING!"
...
echo "MERGED SHEET CHECK: Attendance Report → IN / OUT → VINIT LODI →"
echo "08 Aug cell shows IN/OUT + amber OT line (OT 20:05-08:00 · hrs)."
