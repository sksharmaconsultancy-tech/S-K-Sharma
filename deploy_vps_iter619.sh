#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 619)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 619) ═══════════
#
# ⌨️ KEYBOARD SHORTCUTS PHASE 3 (admin desktop portal):
...
#   wget -O deploy619.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy619.sh
...
echo -n "   Server badge is 619 (must say OK): "
grep -q 'APP_ITERATION = "619"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
