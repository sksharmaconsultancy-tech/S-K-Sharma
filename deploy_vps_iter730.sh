#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 730)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 730) — 🆕 3 नए MODULES: GATE PASS + LATE PENALTY + F&F ═══════════
#
...
#   wget -O deploy730.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy730.sh
...
grep -q 'APP_ITERATION = "730"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
