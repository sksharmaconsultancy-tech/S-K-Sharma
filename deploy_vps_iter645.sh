#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 645)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 645) ═══════════
#
# 📄 PDF REGISTER — dynamic allowance columns (user request):
#  * INCENTIVE (and every custom allowance head ticked in Firm Master)
#    now has its OWN column in BOTH PDF salary register formats
#    (Format 1 & Format 2), split out of OTHER. All totals unchanged.
#
# 🧰 Iter 644 (also included) — Salary Lock fix (validation popup always
#    opens) · INCENTIVE columns on grid + Excel/CSV · OT columns follow
#    the OVER TIME toggle · 643 — Excel-import PF ÷days fix, frozen page
#    headers, note-chips row, 12px grid font · 642-634 earlier fixes.
#
...
echo -n "   Server badge is 645 (must say OK): "
grep -q 'APP_ITERATION = "645"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 645 deployed."
echo "  • NEW (645): INCENTIVE & custom allowance heads now have own"
echo "    columns in BOTH PDF register formats too."
