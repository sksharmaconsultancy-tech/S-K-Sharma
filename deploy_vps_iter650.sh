#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 650)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 650) ═══════════
#
# 🔒 SALARY LOCK — "nothing happens" FIX (user: Gridfree Solar):
#  * A hung save/validate/finalize request could leave the lock stuck,
#    silently swallowing every later click. Every finalize step now has a
#    hard timeout, and every skipped/failed step SHOWS A MESSAGE — the
#    button can never go silent again.
#
# 📊 TOTAL ROW — head-wise fix (user: Net Payable / Total Deduction):
#  * Every figure in the TOTAL row (Basic…Others, Gross, Wage Base, PF,
#    ESI, PT, TDS, Advance, Other, Total Ded., Net) is now the LIVE SUM
#    of the displayed rows — totals can no longer go stale or land under
#    the wrong head after grid edits.
#
# 🧰 Iter 649-644 (also included) — Arrow-key row navigation · Bulk
#    Correction Rate/Pay Basis fix · Lock hardening + editable INCENTIVE
#    · FOOD ALLOWANCE import fix · dynamic allowance columns everywhere.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy650.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy650.sh
...
echo -n "   Server badge is 650 (must say OK): "
grep -q 'APP_ITERATION = "650"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 650 deployed."
echo "  • NEW (650): Salary Lock can never go silent (timeouts + clear"
echo "    messages); TOTAL row now live-sums every head correctly."
