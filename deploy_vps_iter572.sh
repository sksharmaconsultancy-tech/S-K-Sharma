#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 572 — Readable Audit Log + Sub-Admin OTP)
# Deploys the FULL latest code (includes Iter 568-571). Running 572 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 572) ═══════════
#
# A. STEP-BY-STEP READABLE AUDIT LOG:
#    * Every Users Log row now reads like a sentence, e.g.:
#      "S.K. Sharma Admin (Super Admin) updated Employee RAJESH —
#       changed Designation: Helper → Supervisor (+2 more fields)".
#    * Raw IDs (company_id=cmp_..., user_id=...) removed from Details.
#    * "What Happened" line added to the View Details modal; Excel now
#      has a "Description (Step-by-step)" column.
#
# B. SUB-ADMIN LOGIN OTP → SUB USER'S OWN EMAIL:
#    * OTP always goes to the logging-in user's own email.
#    * IMPORTANT: your Resend account is in TEST MODE — it can only
#      deliver to sksharmaconsultancy@gmail.com. Until you verify a
#      domain at resend.com/domains, sub-admin OTPs are auto-forwarded
#      to the Super Admin email (clearly flagged on the OTP screen) so
#      nobody is locked out.
#    * PERMANENT FIX: verify your domain at resend.com/domains, then
#      set RESEND_FROM_EMAIL=no-reply@yourdomain in backend/.env.
#
# C. Clearer red errors on OTP resend failures + env auto-repair.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy572.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy572.sh
...
echo -n "   Server badge is 572 (must say OK): "
grep -q 'APP_ITERATION = "572"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Step-by-step descriptions — Iter 572 (must say OK): "
grep -q '_describe' $APP_DIR/backend/routes/reports_extra.py && echo "OK" || echo "MISSING!"
echo -n "   Sub-admin OTP fallback — Iter 572 (must say OK): "
grep -q 'sent_to_admin' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 572 deployed."
echo "  Test 1: Users Log Report — rows now read as full sentences;"
echo "    Details no longer shows company_id=... codes."
echo "  Test 2: Sub-admin login — OTP goes to their email; in Resend"
echo "    TEST MODE it is forwarded to the Super Admin email (flagged)."
echo "  PERMANENT EMAIL FIX: verify domain at resend.com/domains and"
echo "    set RESEND_FROM_EMAIL in backend/.env."
