#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 532)
#
# ═══════════════════ WHAT'S NEW SINCE 531 ═══════════════════
#
# 1. USER MANUAL — SCREENSHOT / HEADING FIX (user request):
#    Every section now shows the CORRECT screenshot —
#    • Employee Master shows the employee LIST (search + cards),
#      not the Admin Panel stats.
#    • Attendance grid captured for the month that HAS punch data
#      (2026-07) instead of a blank current month.
#    • Monthly Payroll Report captured with real P/A codes and a
#      finalized compliance run.
#    • Bank Transfer captured on the salary-run month (Jul 2026).
#    • Payslip shows a REAL salary (₹22,100) — never a ₹0 slip.
#
# 2. USER MANUAL — COVER PAGE CLEANUP (user request):
#    • "Product By : S.K. Sharma & Co. Payroll & Compliance Portal"
#    • "Prepared By : Ankit Sharma"
#    • REMOVED: Document Version, Last Updated, Screenshots As Of,
#      CONFIDENTIAL line, and the whole "What's New" page + its
#      Table-of-Contents entry.
#    • NEW punch line on EVERY page footer:
#      "Your Satisfaction is our First Ambition"
#
# 3. ROLLBACK (user request): the "Refresh Screenshots /
#    Re-capturing screenshots…" button was removed from the User
#    Manual screen. Fresh, corrected screenshots ship pre-bundled
#    in this deploy.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy532.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy532.sh
...
grep -q 'APP_ITERATION = "532"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo -n "   Corrected manual screenshots bundled (must say 21): "
