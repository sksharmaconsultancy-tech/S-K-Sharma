...
export default function GetAppScreen() {
  ...
  useEffect(() => {
    ...
    // The install prompt usually fires BEFORE React mounts — +html.tsx
    // stashes it on window.__pwaInstallEvt so we can pick it up here.
    const stashed = (window as any).__pwaInstallEvt;
    if (stashed) setInstallEvt(stashed);
    const onReady = () => setInstallEvt((window as any).__pwaInstallEvt || null);
