...
function InlineCompanyPicker({ onPick }: { onPick: (cid: string) => void }) {
  const { companies: allCompanies, selectedCompanyId, companiesLoading } = useSelectedCompany();
  const router = useRouter();
  // Iter 769 (user request) — sirf OFFLINE SALARY enabled firms hi
  // attendance policy set kar sakti hain; baaki firms filter ho jati hain.
  const [offIds, setOffIds] = useState<string[] | null>(null);
  useEffect(() => {
    api<{ offline_company_ids: string[] }>("/attendance/policy/saved-list")
      .then((r) => setOffIds(r.offline_company_ids || []))
      .catch(() => setOffIds(null)); // fetch fail → sab dikha do (fail-open)
  }, []);
  const companies = React.useMemo(
    () => (offIds === null
      ? (allCompanies || [])
      : (allCompanies || []).filter((c) => offIds.includes(c.company_id))),
    [allCompanies, offIds],
  );
  if (companiesLoading) {
    return (
    ...
    return (
      <View style={{ alignItems: "center", gap: 8 }}>
        <Text style={{ color: colors.onSurfaceSecondary, textAlign: "center" }}>
          {offIds !== null && (allCompanies || []).length > 0
            ? "Kisi firm me Offline Salary enabled nahi hai — Attendance Policy sirf Offline/Actual Salary wali firms ke liye set hoti hai (Firm Master → Salary Process me enable karein)."
            : "No firms exist yet. Create one first — the attendance policy is configured per firm."}
        </Text>
        <Pressable
    ...
    <View style={styles.root} testID="attendance-policy-screen">
      ...
      {loading || !policy ? (
        ...
        <KeyboardAwareScrollView bottomOffset={64} contentContainerStyle={styles.scroll}>
          ...
          {/* Iter 769 (user request) — Bulk Dummy Shift Assign ab EMPLOYEE
              MASTER me hai (Attendance Policy se hata diya). */}
...
const PM_FLAGS: { key: string; label: string }[] = [
  ...
  // Iter 270 (user request) — OT Include in Existing Compliance Salary.
...
function PolicyMasterSubPoints({
  ...
  return (
    <View style={pmStyles.card} testID="policy-master-subpoints">
      ...
      {PM_FLAGS.map((f) => {
        const dflt = f.key === "multiple_punch_allowed" || f.key === "geofencing_required" || f.key === "compliance_ot_include";
