...
export default function AttendanceScreen() {
  ...
  // When TRUE, the app runs a background geofence-auto punch flow and we
  // hide the manual Punch In / Out button. When FALSE, the manual button
  // is shown — but the same rules (GPS on + inside geofence) still apply
  // both client- and server-side.
  const autoPunchActive = user?.effective_auto_punch !== false;

  // Iter 64 — GPS punching gate. Server returns `effective_gps_punch`:
  //   • firm_loc_punch (company.location_punching_enabled)   AND
  //   • user_opt_in     (users.gps_punch_enabled)
  // Default is FALSE at both levels. When FALSE, this employee must punch
  // via MANUAL BIOMETRIC ONLY (fingerprint device auth + face selfie).
  // GPS/geofence UI is hidden and lat/lon are never sent.
