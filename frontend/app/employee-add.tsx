...
export default function EmployeeAddScreen() {
  ...
  // Iter 377 (user rule) — Date of Joining auto-fills to the 1st of the
  // current month for a NEW employee (edit mode overwrites it below).
  const [form, setForm] = useState<EmpForm>(() => {
    const now = new Date();
    const mm = String(now.getMonth() + 1).padStart(2, "0");
    return { ...EMPTY_FORM, doj: `01-${mm}-${now.getFullYear()}` };
  });
  ...
  // Iter 376 (user request) — Employee Code input stays DISABLED (auto)
  // until the pencil button unlocks manual entry.
  const [codeManual, setCodeManual] = useState(false);

  const isoToDDMM = (v?: string | null) => {
    const m = (v || "").match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (m) return `${m[3]}-${m[2]}-${m[1]}`;
    // Iter 377 (user bug) — legacy DOJ/DOB stored as DD-MM-YYYY showed
    // BLANK on the edit form; pass the value through unchanged instead.
    const dm = (v || "").replace(/\//g, "-").match(/^(\d{2})-(\d{2})-(\d{4})/);
  ...
  // Iter 376 (user request) — Permanent Address PIN → City/District + State.
  const lookupPermPin = async (p: string) => {
    if (!/^[1-9]\d{5}$/.test(p)) return;
    try {
      const r = await api<{ district: string; state: string }>(`/locations/pincode/${p}`);
      if (r.district) setField("permanent_district", r.district);
      if (r.state) setField("permanent_state", r.state);
    } catch { /* manual entry stays possible */ }
  };
  ...
  const getMandatoryError = (): string | null => {
    ...
    // Iter 375 (user rule) — Employee Code: letters & digits ONLY, and a
    // blank code is auto-generated by the server.
    const _code = form.employee_code.trim();
    if (_code && !/^[A-Za-z0-9]+$/.test(_code))
      return "Employee Code can contain only letters and numbers — no special characters or spaces.";
    // Iter 375 (user rule) — Employee Group is MANDATORY.
    if (!form.employee_type.trim() && !form.employee_group.trim())
      return "Employee Group is required — select the Group.";
  ...
  const submit = async () => {
    ...
    try {
      const payload: any = {
        ...
        // Iter 376 (user bug) — Name as per Aadhaar was NEVER sent to the
        // server, so it vanished every time the entry was reopened.
        aadhaar_name: form.aadhaar_name.trim().toUpperCase() || undefined,
        ...
        // Iter 376 (user request) — Permanent Address City/District + State.
        permanent_district: form.permanent_district.trim() || undefined,
        permanent_state: form.permanent_state.trim() || undefined,
      ...
      >
        ...
        >
                ...
                <Field
                  label={
                    firmHeads.autoCode
                      ...
                      : codeManual
                        ? "Employee Code (manual entry)"
                        : "Employee Code (AUTO — tap ✎ to enter manually)"
                  }
                  value={form.employee_code}
                  onChange={(v) => setField("employee_code", v.replace(/[^A-Za-z0-9]/g, ""))}
                  placeholder="Auto-assigned by system"
                  editable={!firmHeads.autoCode && codeManual}
                />
              </View>
              {/* Iter 376 (user request) — code stays DISABLED (auto) by
                  default; the pencil unlocks manual entry. */}
              {!firmHeads.autoCode ? (
                <Pressable
                  onPress={() => setCodeManual((m) => !m)}
                  style={{
                    padding: 11, borderWidth: 1, borderColor: "#CBD5E1",
                    borderRadius: 8, marginBottom: 2,
                    backgroundColor: codeManual ? "#FEF3C7" : "#F8FAFC",
                  }}
                  testID="emp-code-edit-toggle"
                >
                  <Ionicons
                    name={codeManual ? "lock-open-outline" : "pencil"}
                    size={16}
                    color="#0F3B5C"
                  />
                </Pressable>
          ...
          <Field
            ...
            value={form.name}
            onChange={(v) => {
              const nm = v.toUpperCase();
              // Iter 376 (user request) — Name on PAN / Aadhaar auto-fill
              // from the Employee Name until edited independently.
              setForm((f) => ({
                ...f,
                name: nm,
                pan_name: !f.pan_name.trim() || f.pan_name === f.name ? nm : f.pan_name,
                aadhaar_name: !f.aadhaar_name.trim() || f.aadhaar_name === f.name ? nm : f.aadhaar_name,
              }));
            }}
          ...
          <TwoCol>
            ...
            <Field
              ...
              onChange={(v) => {
                const p = v.replace(/\D/g, "").slice(0, 6);
                setField("permanent_pincode", p);
                if (p.length === 6) void lookupPermPin(p);
              }}
              placeholder="e.g. 311001"
              keyboardType="numeric"
              editable={!sameAsPresent}
          ...
          {/* Iter 376 (user request) — Permanent Address City/District +
              State (auto from the permanent PIN, still editable). */}
          <TwoCol>
            <Field
              label="Permanent City / District"
