...
export function SelectedCompanyProvider({ children }: { children: React.ReactNode }) {
  ...
  const persistLastCompany = useCallback((cid: string | null) => {
    // NOTE: api() stringifies the body itself — passing a pre-stringified
    // body double-encodes it and FastAPI rejects with 422 (the silent bug
    // that made "remember my firm" never save in production).
    void api("/me/last-company", {
      method: "PATCH",
      body: { company_id: cid },
    }).catch(() => { /* best-effort */ });
  }, []);
  ...
  const reloadCompanies = useCallback(async () => {
    ...
    try {
      ...
      // Keep whatever list we already had — wiping it on a transient
      // network failure used to leave the header picker stuck on
      // "No firms" until a full reload.
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  // Iter 124 — After login, default to the firm the admin had selected at
  // their last logout (stored server-side per account). Only when nothing
  // is already selected locally, and only once per login.
  useEffect(() => {
    if (!user || (user.role !== "super_admin" && user.role !== "sub_admin")) return;
    if (companies.length === 0) return;
    if (selectedCompanyId) return;
    if (restoredForUser.current === user.user_id) return;
    restoredForUser.current = user.user_id;
    void (async () => {
      try {
        const r = await api<{ company_id: string | null }>("/me/last-company");
        const cid = r.company_id;
        if (cid && companies.some((c) => c.company_id === cid)) {
          setSelected(cid);
          setIsLocked(true);
          if (Platform.OS === "web") {
            try {
  ...
  }, [isLocked, selectedCompanyId, persistLastCompany]);

  // Iter 123 — Direct firm switch from the header GlobalCompanyPicker.
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      clearLock();
      // Iter 126 FIX — allow the restore to run again on the NEXT login.
      // Without this, logging out and back in as the SAME user (SPA — no
      // full page reload) skipped the server-side "last firm" restore.
