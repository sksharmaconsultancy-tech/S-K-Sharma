...
export default function GlobalCompanyPicker({ compact = false }: { compact?: boolean }) {
  ...
  const commitSelection = (cid: string | null) => {
    // Iter 123 — use switchCompany so an explicit pick from the header
    // dropdown ALWAYS switches the firm (the old setSelectedCompanyId
    // silently no-oped once the session lock was set — the "Switch Firm
    // not working" bug).
    switchCompany(cid);
    if (cid) {
      pushRecent(cid);
