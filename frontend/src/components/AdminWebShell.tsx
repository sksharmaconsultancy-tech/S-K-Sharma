...
export const NAV_SUPER: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      // Iter 114 — back under Utilities as a sub-option (user request).
  ...
  },
  {
    ...
    children: [
      ...
      { route: "/contribution-sheets?kind=pf", label: "P.F. Contribution Sheet", icon: "briefcase-outline" },
      { route: "/contribution-sheets?kind=esi", label: "E.S.I. Contribution Sheet", icon: "medkit-outline" },
      ...
      { route: "/admin", label: "Employee Report", icon: "people-outline" },
    ],
  },
  {
    label: "Automation",
  ...
  },
  {
    ...
    children: [
      ...
      { route: "/employee-groups", label: "Employee Groups", icon: "people-outline" },
    ...
    if (role === "company_admin") {
      ...
      if (empPerms.length === 0) {
        return filterNav(NAV_COMPANY_ADMIN, (item) => {
          ...
          const r = (item.route || "").split("?")[0];
          return !COMPLIANCE_ROUTES.has(r) || hasComplianceGrant;
        });
      }
      return filterNav(NAV_COMPANY_ADMIN, (item) => {
        if (!menuAllowed(item)) return false;
