#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 563 — includes 555→562)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 563) ═══════════
#
# A. PUNCHING API — SECURITY FAILURE EMAIL ALERTS [Iter 563 NEW]:
#    When any API client accumulates 5+ security failures within
#    15 minutes (bad HMAC signature, wrong key, non-whitelisted IP,
#    replay attempts…), the super admin gets an EMAIL alert
#    automatically (max 1 per client per hour) with the client, latest
#    failure reason, count and source IP — so intrusion attempts are
#    flagged immediately. Threshold / alert email configurable per
#    client via the API Integration module.
#
# B. Test Console [562], Secure Punching Push API + Integration
#    Master + Logs + Vendor Docs [561], Performance-sheet import [560],
#    Punch Approvals Excel [559] + Bio Code [558], Time Format 12/24
#    [557], ✎ badge [556], Manual OT fix + Super Admins [555].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy563.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy563.sh
...
echo -n "   Server badge is 563 (must say OK): "
grep -q 'APP_ITERATION = "563"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Security alert engine (must say OK): "
grep -q '_maybe_security_alert' $APP_DIR/backend/routes/punch_push_api.py && echo "OK" || echo "MISSING!"
echo -n "   Test Console — Iter 562 (must say OK): "
grep -q 'test-console' $APP_DIR/backend/routes/punch_push_api.py && echo "OK" || echo "MISSING!"
echo -n "   Punching Push API — Iter 561 (must say OK): "
[ -f $APP_DIR/backend/routes/punch_push_api.py ] && echo "OK" || echo "MISSING!"
echo -n "   RESEND_API_KEY configured (needed for alert emails): "
grep -q '^RESEND_API_KEY=..*' $APP_DIR/backend/.env && echo "OK" || echo "MISSING — alerts will be logged but not emailed"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 563 deployed."
echo "  Security alerts: 5+ API auth failures in 15 min → email to"
echo "  sksharmaconsultancy@gmail.com (max 1/hour per client)."
