#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 556 — includes Iter 555)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 + 556) ═══════════
#
# A. MANUAL OT PUNCH EDIT BUG — FIXED (P0, Anil Purbiya 5th) [Iter 555]:
#    Manually edited/added punches (source "manual_admin") were being
#    deleted by the 5-minute duplicate-punch filter (repair modal saves
#    OT IN 1 min after duty OUT — when the duty OUT was also manual the
#    OT IN was dropped and the whole OT session vanished).
#    FIX: manual admin punches are NEVER deduplicated.
#
# B. 2 MORE SUPER ADMINS — NOW POSSIBLE [Iter 555]:
#    A hidden startup sweep demoted any super admin not in the built-in
#    allowlist back to "employee" on every backend restart. Accounts
#    created via Administration → Super Admin Rights are now exempt and
#    keep super_admin permanently. Also: the list API no longer exposes
#    PIN hashes.
#
# C. "ADMIN CORRECTED" BADGE ON THE ATTENDANCE GRID [Iter 556 — NEW]:
#    Day cells that contain manually edited/added punches now show a
#    violet ✎ badge (legend: "Admin Corrected — manually edited punches
#    (protected)"), so operators instantly see which days carry admin
#    corrections that the engine protects from deduplication.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy556.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy556.sh
...
echo -n "   Server badge is 556 (must say OK): "
grep -q 'APP_ITERATION = "556"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo -n "   Admin-corrected flag in engine (must say OK): "
grep -q 'Iter 556' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Admin-corrected badge in grid UI (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 556 deployed."
echo "  Test 1 (OT fix): open Anil Purbiya 5th — edited OT In/Out shows."
echo "  Test 2 (Super Admins): Administration → Super Admin Rights → Add."
echo "  Test 3 (NEW badge): Attendance Report grid — days with manual"
echo "    corrections show a violet ✎ badge (see the legend line)."
