#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 651)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 651) ═══════════
#
# 🐛 ESIC ON REPROCESS — FIX (user bug):
#  * "Salary Process → With EXISTING data" could leave ESIC computed on
#    the wrong gross until attendance was touched. PF / ESIC / PT now
#    ALWAYS refresh on the kept gross whenever it differs from the fresh
#    calculation — no matter which kept field moved it.
#
# ⌨️ KEYBOARD SHORTCUTS (user request, both salary grids):
#  * ENTER on a highlighted row opens its Present Days cell.
#  * Ctrl+S saves the grid. Ctrl+L opens Finalize & Lock.
#
# 🧰 Iter 650-644 (also included) — silent-lock fix + live TOTAL row ·
...
#   wget -O deploy651.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy651.sh
...
echo -n "   Server badge is 651 (must say OK): "
grep -q 'APP_ITERATION = "651"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 651 deployed."
echo "  • NEW (651): ESIC always recalculated on reprocess-with-existing;"
