#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 548)
#
# ═══════════ WHAT'S NEW SINCE 547 (this deploy) ═══════════
#
# A. IN/OUT SHEET — CELL LAYOUT AS REQUESTED (user request):
#    Each day cell in the IN / OUT sheet now shows, in this exact order:
#        1. In Time            (green)
#        2. Out Time           (red)
#        3. Total Duty HRS
#        4. OT In Time         (amber, OT days only)
#        5. OT Out Time        (amber, OT days only)
#        6. Tot = Duty HRS Including OT  (bold, OT days only)
#    Normal days stay compact (3 lines); OT days grow to 6 lines.
#
#    HOW TO CHECK AFTER DEPLOY: Attendance Report → IN / OUT →
#    VINIT LODI → 08 Aug cell shows:
#        08:00 / 20:00 / 12:00 / OT 20:05 / 08:00 / Tot 23:55
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy548.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy548.sh
...
echo -n "   Server badge is 548 (must say OK): "
grep -q 'APP_ITERATION = "548"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo "CELL LAYOUT CHECK: Attendance Report → IN / OUT → VINIT LODI →"
echo "08 Aug: In / Out / Duty HRS / OT In / OT Out / Tot (incl OT)."
