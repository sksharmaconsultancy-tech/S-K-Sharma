#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 594 — FIRM ACCESS EDITOR + SUPER-ADMIN-ONLY ACCESS TOOLS)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 594) ═══════════
#
# 🏢 ROLES & PERMISSIONS — FIRM ACCESS EDITOR:
#  * New "Firm Access — from Firm Master" block when editing a Sub Admin:
#      all firms from the Firm Master are listed with the user's current
...
#
# 🔒 ACCESS TOOLS NOW SUPER-ADMIN-ONLY (user directive):
#  * "Access Management", "Access Preview", "Roles & Permissions" and
#      "Security · 2FA/MFA" are hidden from every non-super user — people
#      you grant access to can no longer see the tools that grant access.
#
# 📧 EMAIL DELIVERABILITY SELF-CHECK (Security · 2FA/MFA):
#  * New "Run Email Deliverability Check" button — one click reports:
#      Resend key OK? Verified domain? FROM address correct? Own SMTP
#      configured? OTP-via-SMTP toggle? Plus a live test email — and gives
#      the exact fix steps when registered users can't receive OTPs.
#  * IMPORTANT FINDING for your account: smartpayrolling.com is added on
#      Resend but its DNS verification was NEVER COMPLETED (status
#      "not_started"). Finish it at resend.com/domains → open the domain →
#      add the DKIM/SPF records at your DNS provider → wait for "verified"
#      → set RESEND_FROM_EMAIL=no-reply@smartpayrolling.com in backend/.env
#      → restart backend. After that ALL users receive OTP mails.
#
# ⌨️ KEYBOARD SHORTCUTS & KEY NAVIGATION (Phase 1):
#  * Central shortcut engine — ONE global listener, conflict detection,
#      never fires while you are typing in a field.
#  * Global navigation:  Alt+1 Dashboard · Alt+2 Employee Master ·
#      Alt+3 Attendance · Alt+4 Salary Processing · Alt+5 Compliance
#      Salary · Alt+6 Reports · Alt+7 Firm Master · Alt+8 Masters/Settings
#  * Ctrl+K → opens the AI Assistant (which can then navigate, search,
#      download and act by voice/text). ? → searchable Keyboard Shortcuts
#      help overlay (also via the new keypad icon in the header).
#  * Shortcuts respect RBAC exactly like mouse clicks — navigation lands on
#      authz-gated screens, exports/deletes still permission-checked and
#      audited server-side. Touch-only devices are unaffected.
#  * Phase 2 (coming next): page-level Ctrl+S/Ctrl+N/Ctrl+P/Ctrl+Shift+E on
...
#    client admins/staff.
#
# PREVIOUS (Iter 590) — AI direct navigation & voice-first commands.
#
# PREVIOUS (Iter 589) — AI bulk-action engine + bulk undo (preview →
#    approval → execute, CRITICAL risk, Super-Admin-only approval).
#
...
#
# PREVIOUS (Iter 587) — RBAC Phase 2 (export security + Export History) &
#    Phase 3 Maker-Checker with pending-approvals badge and daily 09:00 IST
#    overdue digest email.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy594.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy594.sh
...
echo -n "   Server badge is 594 (must say OK): "
grep -q 'APP_ITERATION = "594"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   AI Bulk engine — Iter 589 (must say OK): "
[ -f $APP_DIR/backend/routes/ai_bulk_actions.py ] && echo "OK" || echo "MISSING!"
echo -n "   AI Command Center backend (must say OK): "
[ -f $APP_DIR/backend/routes/ai_command_center.py ] && echo "OK" || echo "MISSING!"
echo -n "   AI Command Center screen — Iter 588 (must say OK): "
[ -f $APP_DIR/frontend/app/ai-command-center.tsx ] && echo "OK" || echo "MISSING!"
echo -n "   Maker-Checker engine (must say OK): "
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 594 deployed."
echo "  Roles & Permissions now shows all 16 modules x 6 actions with"
echo "  bulk toggles. Client admins/staff now log in with password + OTP."
echo "  NOTE: run the mongo migration below ONCE if you customised 2FA"
echo "  settings earlier (adds client roles to mandatory_roles):"
echo "    mongosh sksharma --eval 'db.security_settings.updateOne({key:\"2fa\", mandatory_roles:{\$exists:true}}, {\$addToSet:{mandatory_roles:{\$each:[\"company_admin\",\"company_staff\"]}}})'"
echo "  Try: 'Kankani me kitne employees hain?', the Alerts tab for"
echo "  missing KYC/duplicates, Insights for payroll KPIs, Activity"
echo "  for the AI audit trail. Approvals tab shows the Maker-Checker"
echo "  queue with risk levels."
