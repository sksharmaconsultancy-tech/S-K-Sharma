#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 549)
#
# ═══════════ WHAT'S NEW SINCE 548 (this deploy) ═══════════
#
# A. IN/OUT EXCEL + PDF EXPORTS NOW MATCH THE MERGED SHEET (user request):
#    • Excel (Grid View): every employee now has 6 rows —
#      D-In, D-Out, T-Hrs (duty only), OT-In, OT-Out (amber),
#      Tot-Hrs (duty INCLUDING OT, bold). The monthly summary shows
#      Duty HRS on T-Hrs, total OT on OT-Out and combined on Tot-Hrs.
#    • PDF: OT days print 6 lines per day cell —
#      In / Out / Duty HRS / OT In / OT Out / T total — normal days
#      stay compact (3 lines), same as the screen.
#    • BONUS FIX: the per-employee "Duty HRS" and "Present Days"
#      summary values in the Excel were printed ONE COLUMN LEFT of
#      their headers (overwriting the last day column, e.g. D31) —
#      now aligned correctly.
#
#    HOW TO CHECK AFTER DEPLOY: Attendance Report → IN / OUT → Excel —
#    VINIT LODI's block must show 6 rows with OT-In 20:05 / OT-Out
#    08:00 / Tot-Hrs on 08 Aug. PDF prints the same 6-line day cell.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy549.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy549.sh
...
echo -n "   Server badge is 549 (must say OK): "
grep -q 'APP_ITERATION = "549"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo "EXPORT CHECK: Attendance Report → IN / OUT → Excel + PDF —"
echo "each employee has 6 rows / 6-line OT day cells matching the screen."
