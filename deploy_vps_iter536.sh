#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 536)
#
# ═══════════════ WHAT'S NEW SINCE 532 (this deploy) ═══════════════
#
# A. MONTHLY PAYROLL REPORT (user request, Iter 536):
...
#
# B. MONTH STEPPER (Iter 536): small ‹ › buttons beside the month box
...
#   wget -O deploy536.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy536.sh
...
grep -q 'APP_ITERATION = "536"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
