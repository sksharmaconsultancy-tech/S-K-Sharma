#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 593 — EMAIL DELIVERABILITY SELF-CHECK (+ Iter 592 shortcuts & OTP fixes))
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 593) ═══════════
#
...
#   wget -O deploy593.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy593.sh
...
echo -n "   Server badge is 593 (must say OK): "
grep -q 'APP_ITERATION = "593"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
