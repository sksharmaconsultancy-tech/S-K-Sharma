#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 576 — MSG91 SMS & OTP, Phase 1)
# Deploys the FULL latest code (includes Iter 568-575). Running 576 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 576) ═══════════
#
# MSG91 SMS & OTP INTEGRATION — PHASE 1:
#   * New screen: Communication → SMS (MSG91). Configure Auth Key
#     (server-side only, masked), Sender ID/DLT header, Entity/PE ID,
#     OTP Flow ID and Default Flow ID — all can be added later when DLT
#     approval completes. Company-wise overrides supported.
#   * Login OTP by SMS: once MSG91 + "OTP via SMS" is enabled, Super/Sub
#     admin 2FA can deliver the 6-digit OTP by SMS (self-generated,
#     hashed; sent through your DLT-approved OTP flow).
#   * Send Test SMS button; SMS Log with masked mobiles (98XXXXXX90),
#     status SENT/FAILED + MSG91 request-id; OTP/SMS events also appear
#     in the Users Log Report (module: Messaging).
#   * Rate limits (configurable): 3 OTP/user/10min, 5 OTP/mobile/hour,
#     10 SMS/user/min.
#   * Provider fully isolated (backend/shared/msg91.py) — payroll and
#     attendance logic untouched. SMS failures NEVER block payroll.
#   * Phase 2 (next): SMS Template Master + salary/attendance/leave
#     event notifications. Phase 3: SMS dashboard + usage report.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy576.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy576.sh
...
echo -n "   Server badge is 576 (must say OK): "
grep -q 'APP_ITERATION = "576"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   MSG91 provider — Iter 576 (must say OK): "
[ -f $APP_DIR/backend/shared/msg91.py ] && echo "OK" || echo "MISSING!"
echo -n "   SMS service + routes — Iter 576 (must say OK): "
[ -f $APP_DIR/backend/routes/sms_notifications.py ] && echo "OK" || echo "MISSING!"
echo -n "   SMS settings UI — Iter 576 (must say OK): "
[ -f $APP_DIR/frontend/app/sms-settings.tsx ] && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 576 deployed (MSG91 Phase 1)."
echo "  Setup: Communication → SMS (MSG91) → enter Auth Key, Sender ID,"
echo "  OTP Flow ID (when DLT approved) → enable SMS + OTP → Send Test."
