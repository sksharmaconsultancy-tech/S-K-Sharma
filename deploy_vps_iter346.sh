#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 346)
# CRITICAL FIX on top of 345 (user: "Still showing old salary structure"):
#   • The old DB structure query filtered EmployeeSalaryStructureDtl by
#     FirmID_FK — which does NOT match our firm number for many firms, so
#     the query returned NOTHING and employees silently kept their stale
#     EmployeeMaster figures. Both the import AND the all-firms sync now
#     JOIN via EmpID_FK → EmployeeMaster.FirmNo (the key that is verified
#     correct everywhere else), so structures are found for every firm.
#   • The sync also clears stale per-head fields (hra_amount / conv_amount /
#     basic_amount / old salary_structure_compliance) so nothing double
#     counts on top of the fresh allowance list.
#   • Sync job now reports structure_rows per run and flags any firm with
#     ZERO structure rows in the old DB.
#   → AFTER DEPLOY: Legacy Import → press "Sync Salary Structures — ALL
#     Firms (from Old DB)" AGAIN. Check employees updated / gross changed.
#   • GROSS FIGURE CORRECTNESS (Employee Master + emailed Attendance Sheet
...
echo ""
echo "✅ Deploy Iter 346 complete! HARD-REFRESH the browser (Ctrl+Shift+R)."
