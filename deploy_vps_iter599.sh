#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 599 — VAULT ACCESS LOG for Firms ID & Password)
# Deploys the FULL latest code (includes ALL of Iter 568-598). Running 599 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 599) ═══════════
#
# 🛡️ VAULT ACCESS LOG (Firms ID & Password):
#  * Every unlock attempt — SUCCESS and FAILED — is now recorded with the
#      admin's name, role, reason (wrong PIN / no PIN set), IP and time.
#  * New "Access Log" button on the Firms ID & Password screen (Super
#      Admin only) shows the last 200 attempts at a glance — great audit
#      evidence for security reviews.
#  * Sub Super Admins cannot view the log (strict Super Admin gate).
#
# PREVIOUS (Iter 598) — Sub Super Admin PIN access fixed: own-PIN hint,
#    clear no-PIN/wrong-PIN errors, "Forgot PIN?" temp-PIN email button.
# PREVIOUS (Iter 597) — Contractor PF Calculation Rule in PF Settings.
# PREVIOUS (Iter 596) — Challan Uploads Month+Group auto-select & merge.
# PREVIOUS (Iter 595) — CURRENT FIRM pinned in header picker; keyboard
#    Phase 2 (grid arrows/Enter/P/A, F5 salary recalculate).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy599.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy599.sh
...
echo -n "   Server badge is 599 (must say OK): "
grep -q 'APP_ITERATION = "599"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
