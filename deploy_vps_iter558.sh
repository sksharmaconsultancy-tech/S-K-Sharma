#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 558 — includes 555+556+557)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 555 → 558) ═══════════
#
# A. PUNCH APPROVALS — COLUMNS FIXED + BIO CODE [Iter 558 NEW]:
#    • Name / Father Name / Designation columns were cut off with "…"
#      — now WIDER (190/170/150 px) and wrap to 2 lines, so full names
#      like "ADITYA KUMAR CHOURASIA" show properly.
#    • NEW "Bio Code" column (machine user ID) added to ALL Punch
#      Approval tables: Updated / Auto-Punches / Manual Entries /
#      Additional Duty and the status tables.
#    • The search box now also searches by Bio Code.
#
# B. TIME FORMAT dropdown (12 Hr AM/PM default / 24 Hr) in Daily
#    In/Out & OT Verification + all its exports [Iter 557].
# C. MANUAL OT PUNCH EDIT BUG FIXED — manual punches never deduped
#    (Anil Purbiya 5th case) [Iter 555].
# D. 2 MORE SUPER ADMINS — survive restarts now [Iter 555].
# E. "ADMIN CORRECTED" ✎ badge on attendance-grid day cells [Iter 556].
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy558.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy558.sh
...
echo -n "   Server badge is 558 (must say OK): "
grep -q 'APP_ITERATION = "558"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Bio Code column in Punch Approvals (must say OK): "
grep -q 'txt: "Bio Code"' $APP_DIR/frontend/app/punch-approvals.tsx && echo "OK" || echo "MISSING!"
echo -n "   Wider Name/Father/Designation columns (must say OK): "
grep -q '{ w: 190, txt: "Name" }' $APP_DIR/frontend/app/punch-approvals.tsx && echo "OK" || echo "MISSING!"
echo -n "   bio_code in day-status API (must say OK): "
grep -q '"bio_code": e.get("bio_code")' $APP_DIR/backend/routes/attendance_admin_core.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 558 deployed."
echo "  Test: Punch Approvals → Auto-Punches / Manual / Updated →"
echo "    full Name, Father Name, Designation visible (2-line wrap),"
echo "    new Bio Code column after Code; search works by bio code."
echo "  Plus all Iter 555–557 items if not yet verified."
