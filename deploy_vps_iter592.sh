#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 592 — KEYBOARD SHORTCUTS & KEY NAVIGATION)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 592) ═══════════
#
...
#   wget -O deploy592.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy592.sh
...
echo -n "   Server badge is 592 (must say OK): "
grep -q 'APP_ITERATION = "592"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
