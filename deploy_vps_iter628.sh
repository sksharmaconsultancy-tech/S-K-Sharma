#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 628)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 628) ═══════════
#
# 🕐 DUMMY SHIFT IN/OUT MATRIX REPORT (user spec, 18 sections):
#  * The In/Out & OT Matrix screen gets a "Dummy Shift Mode" toggle —
#    visible ONLY when the firm has "Dummy Shift Allowed" enabled in
#    the Attendance Policy (Firm Master activation, sec 1).
#  * REPORT-ONLY LAYER (sec 7/17): generating, refreshing, exporting or
#    printing NEVER writes to attendance, punches, shifts, payroll,
#    compliance salary, OT, leave or the Employee Master. All values
#    are computed at display time from the same grid engine.
#  * Day-status priority (sec 10): Holiday → H, Week Off → WO (from the
#    employee's own week-off configuration + firm Holiday Master),
#    then Present. No dummy timings are generated on H / WO days.
#  * Punch-count logic (sec 6): 2 punches → Dummy Shift master IN/OUT
#    timings; 1 punch → the available punch, missing side per the
#    existing matrix logic; >2 punches → existing representation kept.
#  * Overnight shifts (sec 11): OUT displayed with a * (e.g. 00:00*) —
#    the OUT belongs to the NEXT calendar date; never counted as an
#    extra attendance day.
#  * Reuses the EXISTING payroll matrix layout (sec 12): same rows
#    (D-In/D-Out/Hrs/OT), day columns, colours, landscape PDF, Excel,
#    CSV, print, filters + a new Dummy Shift filter.
#  * Header shows "DUMMY SHIFT IN / OUT MATRIX REPORT" + red
#    "FOR REPORTING PURPOSE ONLY" disclaimer (sec 15) and a summary
#    block (sec 16): active employees, present/WO/holiday/absent day
#    counts, shift-wise employee counts.
#
# 📊 Iter 627 (also included) — SHIFT DEPLOYMENT: SUMMARY ONLY option
#    (Department-wise + Designation-wise totals, no employee rows).
#
# (All Iter 618-626 features also included.)
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy628.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy628.sh
...
echo -n "   Server badge is 628 (must say OK): "
grep -q 'APP_ITERATION = "628"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 628 deployed."
echo "  • NEW (628): DUMMY SHIFT IN/OUT MATRIX — open In/Out & OT"
