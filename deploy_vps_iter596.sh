#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 596 — CHALLAN UPLOADS: MONTH + GROUP AUTO-SELECT, ALL GROUPS MERGED)
# Deploys the FULL latest code (includes ALL of Iter 568-595). Running 596 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 596) ═══════════
#
# 🏛️ PF / ESIC CHALLAN UPLOADS — SIMPLER RUN SELECTION:
#  * The separate "Compliance Run" dropdown is REMOVED — just pick the
#      Month and the Employee Group; the newest FINALIZED run is selected
#      automatically (green "Selected Run (auto)" chip confirms it).
#  * Only FINALIZED compliance runs are offered — drafts never show.
#  * Employee Group defaults to "All groups": every finalized group of the
#      month is MERGED into ONE portal file (ECR .txt/.xlsx, ESIC .xls/.xlsx,
#      previews, EPFO File Check and Auto-Upload all use the merged data;
#      duplicate employees deduped).
#
# PREVIOUS (Iter 595) — header firm dropdown pins CURRENT FIRM (OPEN) on
#    top + filter; keyboard Phase 2: attendance grid ↑↓←→/Enter/P/A keys
#    (audited quick-mark API), F5 recalculate on salary screens.
#
# PREVIOUS (Iter 591-594) — 16-module permission matrix, data-scope editor
#    with filters, OTP login for client admins, keyboard shortcuts Phase 1,
#    email deliverability self-check.
#
# PREVIOUS (Iter 587-590) — RBAC phase 2/3, Maker-Checker, AI Command
#    Center, AI bulk actions + undo, AI navigation & voice commands.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy596.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy596.sh
...
echo -n "   Server badge is 596 (must say OK): "
grep -q 'APP_ITERATION = "596"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
