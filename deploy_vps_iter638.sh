#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 638)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 638) ═══════════
#
# 🧰 CONFIGURE BATCH — LAYOUT FIXES (user feedback on Iter 637):
#  * Columns were overlapping each other (the Month/FY selectors were
#    colliding with the "Month Days" label) — the Month column now has
#    a proper 300px width and every column clips cleanly; nothing
#    overlaps at any screen width.
#  * Summary cards (Total / Processed / Pending) now always fit on the
#    SAME single line as the filters.
#  * "Salary Process" (green) and "Copy Last Month Salary" (purple)
#    are now COMPACT side-by-side buttons on one line — no more
#    full-width stretched bars.
#
# 🧰 Iter 637 (also included) — Configure Batch single-line toolbar ·
#    636 — Actual grid redesign + compact header · 635 — grid fonts.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy638.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy638.sh
...
echo -n "   Server badge is 638 (must say OK): "
grep -q 'APP_ITERATION = "638"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 638 deployed."
echo "  • NEW (638): toolbar overlap FIXED — Month/FY selectors no"
