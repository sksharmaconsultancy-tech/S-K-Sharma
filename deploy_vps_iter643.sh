#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 643)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 643) ═══════════
#
# 🐛 EXCEL IMPORT — PF month-days FIX (user bug):
#  * Importing a salary sheet for a FRESH month no longer defaults the
#    PF divisor to the calendar days (31). The Firm Master's Fixed
#    Days setting (e.g. 26) now applies to imported-sheet runs too.
#    A typed Month Days (Override) still always wins.
#
# 📌 COMPLIANCE SALARY page (user requests, VIEW-ONLY):
#  * Select Firm + Configure Batch cards are FROZEN at the top of the
#    page — they never scroll away.
#  * MASTER SNAPSHOT / PF ÷days / FREEZE / COPIED note-chips moved to
#    their OWN line above the action buttons.
#  * Salary grid font reduced by 2 (14px → 12px) with tighter
#    auto-fit columns so more data fits on screen.
#
# 🧰 Iter 642-641 (also included) — Configure Batch single-line filter
#    alignment · Salary Process two-line compact button · 640-634 —
#    earlier toolbar/grid redesigns, Approve Backlog, autosave fix.
#
...
echo -n "   Server badge is 643 (must say OK): "
grep -q 'APP_ITERATION = "643"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 643 deployed."
echo "  • NEW (643): Excel-import PF fix — fresh imported months use"
echo "    the Firm Master Fixed Days (26) instead of calendar 31."
echo "  • NEW (643): Select Firm + Configure Batch frozen on top;"
echo "    note-chips on their own line; grid font 12px."
