#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 513) — MACHINE-vs-MASTER LIST FIX + SDK PULL
#
...
#   wget -O deploy513.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy513.sh
...
echo -n "   Server badge is 513 (must say OK): "
grep -q 'APP_ITERATION = "513"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "   VERIFY: footer badge must read 'Server Iter 513'."
