#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 641)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 641) ═══════════
#
# 🧰 CONFIGURE BATCH — button polish (user feedback):
#  * "Salary Process" (green) is now a compact TWO-LINE button, same
#    size style as "Copy Last Month Salary". Same action, no logic
#    changes.
#
# 🧰 Iter 640 (also included) — Month Days rename + narrow input,
#    two-line Copy button · 639 — Approve Backlog + Attendance Grid
#    fonts · 638/637 — Configure Batch toolbar · 636-635 — grid
#    redesigns · 634 — autosave + multi-tab fix.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy641.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy641.sh
...
echo -n "   Server badge is 641 (must say OK): "
grep -q 'APP_ITERATION = "641"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 641 deployed."
echo "  • NEW (641): Salary Process button is now a compact two-line"
