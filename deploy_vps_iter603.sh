#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 603 — PF BASIC AUTO-COPY FIX + REFRESH MASTER KEEPS SAVED DATA)
# Deploys the FULL latest code (includes ALL of Iter 568-602). Running 603 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 603) ═══════════
#
# 🐞 EMPLOYEE MASTER — PF BASIC AUTO-COPY BUG FIXED:
#  * While typing a Basic Salary like 25000, the partial value "2500"
#      (below the ₹15,000 ceiling) auto-copied into PF Basic Salary and
#      stayed there. Now: once Basic reaches/crosses ₹15,000 the PF Basic
#      field is CLEARED automatically (blank by default) — admin can still
#      type a PF Basic manually if the firm adopts one.
#
# 🐞 COMPLIANCE SALARY PROCESS — "REFRESH MASTER" NO LONGER WIPES DATA:
#  * Clicking "Refresh Master" was resetting ALL saved grid data (entered
#      present days, manual OT / Others / TDS / Advance amounts).
#  * Now it refreshes ONLY the master snapshot (rates, basic, policies) —
#      every manually entered/saved figure is preserved via the
#      non-destructive reprocess engine. Tested: others ₹777 + 9.5 days
#      survived a master refresh unchanged.
#
# PREVIOUS (Iter 601-602) — Secure Punch: WebAuthn device lock, HR face
#    enrollment, liveness + anti-spoof + 1:1 face match punch flow.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy603.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy603.sh
...
echo -n "   Server badge is 603 (must say OK): "
grep -q 'APP_ITERATION = "603"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
