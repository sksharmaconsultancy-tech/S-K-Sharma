#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 629)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 629) ═══════════
#
# 🕐 EMPLOYEE PWA — PUNCH TIME SHOWING WRONG (user bug, FIXED):
#  * Punches are stored as IST wall-clock (labelled UTC). The "My
#    Attendance & Shift" screen converted them to Asia/Kolkata AGAIN,
#    adding +5:30 twice — a 09:57 AM punch displayed as 03:27 PM.
#  * Fixed: times now render exactly as punched (UTC render of the
#    stored wall-clock). Attendance-correction requests also fixed —
#    they were being saved with a +05:30 label that made the corrected
#    punch land 5½ hours early on the grid.
#
# ✅ ALL EMPLOYEE-PWA PUNCHES AUTO-APPROVE (user request):
#  * "Auto-approve Mobile App Punches" now defaults ON for every firm
#    (migration below flips all existing firms ON). Every PWA punch
#    lands APPROVED instantly — no admin approval queue.
#  * A firm can still switch it OFF in Firm Settings if ever needed.
#  * Safety kept: fake/mock-GPS flagged punches ALWAYS need manual
#    approval; onboarding-eligibility holds & contractual gates remain.
#
# 🕐 Iter 628 (also included) — DUMMY SHIFT IN/OUT MATRIX (web portal
#    only) · Iter 627 — Shift Deployment SUMMARY ONLY option.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy629.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy629.sh
...
echo -n "   Server badge is 629 (must say OK): "
grep -q 'APP_ITERATION = "629"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 629 deployed."
echo "  • NEW (629): PWA punch time FIXED — My Attendance was adding"
...
echo "  • NEW (629): ALL employee PWA punches now AUTO-APPROVE by"
