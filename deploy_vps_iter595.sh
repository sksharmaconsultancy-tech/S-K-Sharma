#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 595 — HEADER FIRM PICKER: CURRENT FIRM PINNED ON TOP)
# Deploys the FULL latest code (includes ALL of Iter 568-594). Running 595 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 595) ═══════════
#
# 🏢 HEADER FIRM SELECTION — CURRENT/OPEN FIRM ALWAYS VISIBLE:
#  * Opening the firm dropdown in the top header now shows a pinned
#      "CURRENT FIRM (OPEN)" section at the very top with the firm you are
#      working in highlighted and check-marked — you always see instantly
#      which firm is open.
#  * 🔍 FILTER: the search box narrows firms by name OR code as you type
#      (e.g. "kank" → Kankani Enterprises).
#  * Keyboard: ↑↓ move, Enter picks (cursor starts on the open firm),
#      Esc closes. RECENT firms and the full ALL FIRMS list follow below.
#
# PREVIOUS (Iter 594) — Data-scope editor: firm/branch/department pickers
#    with filters; access tools restricted to Super Admin; email
#    deliverability self-check; keyboard shortcuts Phase 1.
#
# PREVIOUS (Iter 591-593) — 16-module permission matrix, OTP login for
#    client admins/staff + verify-screen delivery diagnostics.
#
# PREVIOUS (Iter 588-590) — AI Command Center, AI bulk actions + undo,
#    AI direct navigation & voice commands.
#
# PREVIOUS (Iter 587) — RBAC Phase 2/3: export security, Maker-Checker,
#    daily pending-approvals digest email.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy595.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy595.sh
...
echo -n "   Server badge is 595 (must say OK): "
grep -q 'APP_ITERATION = "595"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
