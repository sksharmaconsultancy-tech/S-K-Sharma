#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 568 — Detailed Audit Trail)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 568) ═══════════
#
# A. DETAILED AUDIT TRAIL — USERS LOG REPORT UPGRADE [Iter 568 NEW]:
#    * Field-level OLD → NEW value tracking for every UPDATE/DELETE on
#      Employees, Attendance, Firms, Sub-admins, Advances, Leaves,
#      Shifts, Masters, Employee Groups and Punching-API clients.
#    * Every log entry now records: Module, Record, IP address, Device
#      (browser), HTTP status and a Success/FAILED flag.
#    * Users Log Report UI: summary cards (Total/Created/Updated/
#      Deleted/Logins/Failed), instant quick filters (action type,
#      status, module), free-text search, and a "View Details" modal
#      showing the full old → new field diff table.
#    * Excel export upgraded: Module, Type, Record, Changes (Old → New),
#      IP and Status columns; failed rows highlighted red.
#    * Multi-firm scope unchanged: firm admins see ONLY their firm.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy568.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy568.sh
...
echo -n "   Server badge is 568 (must say OK): "
grep -q 'APP_ITERATION = "568"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Audit helper module — Iter 568 (must say OK): "
[ -f $APP_DIR/backend/shared/audit.py ] && echo "OK" || echo "MISSING!"
echo -n "   Field-diff middleware — Iter 568 (must say OK): "
grep -q '_audit_changes' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   Users-log summary API — Iter 568 (must say OK): "
grep -q '"unique_users"' $APP_DIR/backend/routes/reports_extra.py && echo "OK" || echo "MISSING!"
echo -n "   Details modal in UI — Iter 568 (must say OK): "
grep -q 'ulr-detail-close' $APP_DIR/frontend/app/users-log-report.tsx && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 568 deployed."
echo "  Test 1: Reports → Users Log Report → summary cards on top,"
echo "    use Audit Filters (type/status/module/search)."
echo "  Test 2: Edit any employee field, reload the log — tap the row"
echo "    to open View Details and see the OLD → NEW field diff."
echo "  Test 3: Excel Report now has Module/Type/Changes/IP/Status."
