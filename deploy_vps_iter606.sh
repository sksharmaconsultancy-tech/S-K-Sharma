#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 606 — Expense Claims COMPLETE + Sales Proposals super-admin-only)
# Deploys the FULL latest code (includes ALL of Iter 568-605). Running 606 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 605 + 606) ═══════════
#
# 🔒 Iter 606 — ACCESS GATING (user directive):
#  * Sales · Proposals is now SUPER-ADMIN-ONLY: removed from the Company
#      Admin sidebar, hidden for Sub Super Admin staff, page redirects
#      non-super-admins, and ALL /api/admin/proposals APIs reject every
#      role except super_admin.
#  * Access & Workflow security screens (Access Management, Access
#      Preview, Roles & Permissions, Security 2FA) remain hidden for
#      staff users (since Iter 594) — confirmed intact.
#
...
#   wget -O deploy606.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy606.sh
...
echo -n "   Server badge is 606 (must say OK): "
grep -q 'APP_ITERATION = "606"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 606 deployed. EXPENSE CLAIMS MODULE IS COMPLETE:"
echo "  • Employees: dashboard → Expense Claim card → New Claim →"
echo "    upload receipt (AI auto-fills) → Save & Submit."
echo "  • Admins: sidebar Payroll → 'Expense Approvals & Payments'"
echo "    (Manager → Accounts → Finance → Payment) and"
echo "    'Expense Reports & Categories' (monthly reports, category"
echo "    master, payroll reimbursement feed)."
