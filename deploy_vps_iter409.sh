#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 409)
# INCLUDES Iter 400-408 (Runner v9, Reports Center alignment, Compliance
# register tally fixes, In/Out & OT Matrix, QR dropdown + PNG, PF/ESIC on
# Gross incl OT, Lock override, Higher PF & VPF module) + NEW IN THIS
# RELEASE:
#
# Iter 409 — REPROCESS 422 FIX:
#   • POST /admin/compliance-salary-runs/{id}/reprocess no longer fails
#     with a 422 validation error when the request body is empty or is
#     missing the "month" field — any missing parameter now falls back
#     to the run's existing saved value (month, month_days, filters,
#     structure %, statutory config). Frontend Reprocess works in all
#     cases.
#
...
echo ""
echo "✅ Deploy Iter 409 complete! HARD-REFRESH the browser (Ctrl+Shift+R)."
echo ""
echo "   VERIFY: Compliance Salary → open any UNLOCKED run → Reprocess —"
echo "   it should recompute without any error in every case."
