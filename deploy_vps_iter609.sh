#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 609 — Iter 608 fixes + client brochure endpoint)
# Deploys the FULL latest code (includes ALL of Iter 568-608). Running 609 alone is enough.
#
...
#   wget -O deploy609.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy609.sh
...
echo -n "   Server badge is 609 (must say OK): "
grep -q 'APP_ITERATION = "609"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
