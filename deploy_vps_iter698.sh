#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 698)
...
#   wget -O deploy698.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy698.sh
...
echo -n "   Server badge is 698 (must say OK): "
grep -q 'APP_ITERATION = "698"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q '_dup_epfo_login_warning' $APP_DIR/backend/routes/portal_extension.py && grep -q 'EPFO_PC_ACTION' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'GuardedGridInput' $APP_DIR/frontend/app/firm-master.tsx && grep -q 'RUNNER_VERSION = "24"' $APP_DIR/backend/routes/portal_extension.py && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "24"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v16' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'typeof body === "string" ? body' $APP_DIR/frontend/src/api/client.ts && grep -q 'sks-pwa-v16' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   Month dropdown + TEST removed + popup auto-close v24 — Iter 698 (must say OK): "
grep -q 'as-month-dd' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'btnCloseModal' $APP_DIR/backend/routes/portal_extension.py && grep -q 'RUNNER_VERSION = "24"' $APP_DIR/backend/routes/portal_extension.py && ! grep -q '"epfo_ecr_autoupload_test": {' $APP_DIR/backend/utils/rpa_engine.py && echo "OK" || echo "MISSING!"
