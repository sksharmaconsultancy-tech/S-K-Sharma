#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 543)
#
# ═══════════ WHAT'S NEW SINCE 537 (this deploy) ═══════════
#
# A. PORTAL HANG FIX (user report, Iter 538):
#    • Monthly Payroll Report responses are now CACHED for 2 min —
#      repeat opens / month-stepper / filters no longer recompute the
#      whole firm's payroll each time (2nd hit: 0.03s vs full compute).
#    • The report was silently loading TWICE on every open — fixed.
#    • Very large firms render rows in chunks of 150 with a
#      "Show more" button instead of freezing the browser.
#
# B. PUNCH-SEQUENCE ATTENDANCE (user rule, Iter 543) — activates when
#    the Attendance-Policy sub-point "Attendance Calculation as per
#    Duty HRS (Days = Total Duty HRS ÷ Daily Duty HRS)" is ON:
#    • duplicate punches of the same kind within 5 min are IGNORED;
#    • 1st IN → OUT pair = Duty, split DYNAMICALLY at the Firm Master
#      Daily Duty HRS (12 worked hrs @ 8-hr firm = 8 duty + 4 OT) —
#      never a hardcoded 8, even when the resolved shift is longer;
#    • any punch AFTER the OUT = OT IN; its partner OUT — even the
#      NEXT-DAY MORNING out punch — = OT OUT (pure OT, no top-up);
#    • Days = Total Duty HRS ÷ Firm Master Daily Duty HRS.
#    Applies everywhere: Attendance Grid, Daily In/Out & OT
#    Verification, OT Report, Monthly Payroll Report, salary runs.
#    Firms with the sub-point OFF are completely unchanged.
#
# C. RULE BADGE (Iter 543): the Daily In/Out & OT Verification header
#    now shows the firm's calculation mode — green "Rule: Punch
#    Sequence" when the sub-point is ON, blue "Rule: Standard
#    (Shift HRS)" otherwise.
#
# D. DEVICE SYNC ENGINE (Iter 543, user request):
#    • "Remove from machines" section — enter Emp Code / Bio Code and
#      queue DATA DELETE USERINFO on every sync-enabled machine.
#    • "Remove All LEFT Employees" — one click deletes every employee
#      marked LEFT (exit/resign date, resigned status or inactive) from
#      the machines (with a named confirmation first).
#      Portal data is never touched — only the machines.
#
# E. ESIC LEAVE (Iter 543, user requests):
#    • "Module Settings" panel hidden from the screen.
#    • Submit ESIC Leave = AUTO-APPROVED (no approval step); attendance
#      auto-marks immediately, submitter recorded as approver.
#
# F. DAILY IN/OUT & OT VERIFICATION (Iter 543, user requests):
#    • Filters Category / Shift / Punch Source removed;
#      "Unit / Group" renamed to "Group".
#    • PDF (Portrait) now has proper 12 mm left/right margins.
#
# G. "DELETED FROM MACHINE ✓" TAG (Iter 543): when a machine confirms a
#    delete command, the employee gets a green "Deleted from machine ✓"
#    tag in Employee Master (All Employee Data). A later re-sync
#    (add/update) clears the tag automatically.
#
# H. PUNCH-PHOTO DIAGNOSTICS + FIXES (Iter 543, open user issue):
#    • Every photo push from a machine is now COUNTED on the device
#      card: "PUNCH PHOTOS  2/3 saved · 5m ago" (Biometric Devices).
#      If a photo can't be read, the card shows the exact reason and
#      the raw header the machine sent — send me a screenshot of that.
#    • Photo filename WITHOUT the "PIN=" prefix now recognised too.
#    • NEW /iclock/fdata endpoint — some eSSL/ZKTeco firmware uploads
#      photos there instead of cdata; both paths now save photos.
#    HOW TO CHECK AFTER DEPLOY: make a test punch in front of the
#    machine → open Devices → the PUNCH PHOTOS counter must increase.
#    If it stays "None received", the machine itself is not sending
#    photos — enable "Capture / take photo on punch" in the machine
#    menu (Setting → Access/Attendance → Camera Mode → take photo).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy543.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy543.sh
...
grep -q 'APP_ITERATION = "543"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
