#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 703)
...
#   wget -O deploy703.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy703.sh
...
echo -n "   Server badge is 703 (must say OK): "
grep -q 'APP_ITERATION = "703"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q '_dup_epfo_login_warning' $APP_DIR/backend/routes/portal_extension.py && grep -q 'EPFO_PC_ACTION' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'GuardedGridInput' $APP_DIR/frontend/app/firm-master.tsx && grep -q 'RUNNER_VERSION = "27"' $APP_DIR/backend/routes/portal_extension.py && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "27"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'typeof body === "string" ? body' $APP_DIR/frontend/src/api/client.ts && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'auto_captured' $APP_DIR/backend/routes/portal_extension.py && grep -q 'auto_captured' $APP_DIR/frontend/app/challans.tsx && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'ecr_attached' $APP_DIR/backend/routes/portal_extension.py && grep -q 'ecr_attached' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'RUNNER_VERSION = "27"' $APP_DIR/backend/routes/portal_extension.py && grep -q '"label": "ECR Upload"' $APP_DIR/backend/utils/rpa_engine.py && echo "OK" || echo "MISSING!"
echo -n "   ESIC same login process v26 — Iter 700 (must say OK): "
grep -q 'esic_open' $APP_DIR/backend/routes/portal_extension.py && grep -q '_diagnose_esic_creds' $APP_DIR/backend/routes/portal_extension.py && grep -q 'esic_open' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   TRRN + challan PDF + ESIC nav v27 — Iter 701 (must say OK): "
grep -q 'portal-ext/trrn' $APP_DIR/backend/routes/portal_extension.py && grep -q 'challan-pdf' $APP_DIR/backend/routes/portal_extension.py && grep -q 'as-download-challan' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'RUNNER_VERSION = "27"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
echo -n "   In/Out report grid cache + debounce — Iter 702 (must say OK): "
grep -q '_GRID_CACHE' $APP_DIR/backend/routes/inout_ot_matrix.py && grep -q 'qDeb' $APP_DIR/frontend/app/inout-ot-matrix.tsx && grep -q 'sks-pwa-v21' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
