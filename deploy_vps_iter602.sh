#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 602 — SECURE PUNCH Phase 2: LIVENESS + ANTI-SPOOF + 1:1 FACE MATCH)
# Deploys the FULL latest code (includes ALL of Iter 568-601). Running 602 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 602) ═══════════
#
# 📸 SECURE FACE PUNCH — VERIFICATION FLOW (server is FINAL authority):
#  * NEW employee screen "/secure-punch?kind=in|out":
#      device auth (if registered) → live camera → RANDOM liveness
#      challenges (turn LEFT/RIGHT, move CLOSER — server-verified from
#      face keypoints, never a frontend boolean) → anti-spoof analysis
#      (moire/screen detection, print flatness, static-replay motion
#      check; dedicated PAD ONNX model auto-used if installed at
#      backend/models/antispoof.onnx) → 1:1 ArcFace match vs the
#      registered template → punch.
#  * /api/attendance/punch now REQUIRES a valid single-use verification
#      session when the firm enables secure_face_punch_enabled — photos,
#      screenshots, replayed videos or another person CANNOT punch.
#  * Failed-attempt lockout (3 fails → 30 min, server-side) and a
#      tamper-resistant punch_verification_audit log with match scores.
#  * Firm flags (Phase 3 UI coming): secure_face_punch_enabled,
#      face_match_threshold_pct (default 72), punch_max_failed_attempts,
#      punch_retry_lock_minutes, secure_punch_webauthn_required,
#      secure_punch_liveness, secure_punch_anti_spoof.
#  * Lab-tested: genuine 100% match PASS · wrong person 53% REJECTED ·
#      punch without verification 403 · lockout 429 · audit rows written.
#
# PREVIOUS (Iter 601) — WebAuthn device lock + HR-only face enrollment
#    (⚠ downloads ~300MB AI models on first deploy).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy602.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy602.sh
...
echo -n "   Server badge is 602 (must say OK): "
grep -q 'APP_ITERATION = "602"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
