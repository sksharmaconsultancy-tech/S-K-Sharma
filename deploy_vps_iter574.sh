#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 574 — OTP Email: Both Options Ready)
# Deploys the FULL latest code (includes Iter 568-573). Running 574 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 574) ═══════════
#
# You chose BOTH email options — this build makes each work automatically:
#
# A. RESEND DOMAIN (recommended): this deploy sets
#      RESEND_FROM_EMAIL=no-reply@smartpayrolling.com
#    in backend/.env. SAFE: until you finish verifying the domain at
#    resend.com/domains, the app auto-falls back to the test sender so
#    Super Admin login keeps working. The MOMENT Resend shows the domain
#    as Verified, OTPs start delivering to EVERY sub user automatically —
#    no further deploy needed.
#
# B. GMAIL SMTP AUTO-RESCUE: if Resend cannot deliver to a sub user
#    (test mode), the app now AUTOMATICALLY sends the OTP through your
#    SMTP (Communication → Email SMTP & Notifications) the moment it is
#    configured & enabled — even without touching the Security toggle.
#    (Toggle "Send OTP via own SMTP" makes SMTP the primary sender.)
#
# SETUP CHECKLIST (one time):
#   [ ] resend.com → Domains → add smartpayrolling.com → add 3 DNS records
#   [ ] Gmail App Password → portal → Email SMTP & Notifications → Save + Test
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy574.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy574.sh
...
echo -n "   Server badge is 574 (must say OK): "
grep -q 'APP_ITERATION = "574"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   From-domain auto-fallback — Iter 574 (must say OK): "
grep -q 'from-domain not verified' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   SMTP auto-rescue — Iter 574 (must say OK): "
grep -q '_try_smtp' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   RESEND_FROM_EMAIL in .env (must say OK): "
grep -q '^RESEND_FROM_EMAIL=' $APP_DIR/backend/.env && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 574 deployed."
echo "  1) Verify smartpayrolling.com at resend.com/domains →"
echo "     sub-user OTPs deliver automatically once Verified."
echo "  2) AND/OR configure Gmail SMTP in Email SMTP & Notifications →"
echo "     OTPs auto-rescue through SMTP immediately."
