#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 769)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ══════ WHAT'S NEW (Iter 769) — 🕐 BULK DUMMY IN EMPLOYEE MASTER + 🎛 FILTERS ══════
#
# 1) BULK DUMMY SHIFT ASSIGN — AB EMPLOYEE MASTER ME (USER REQUEST):
#  * Attendance Policy screen se hata diya; ab Employee Master me hi
#    (Dummy Shift card ke neeche) "Bulk Dummy Shift Assign" milta hai.
#  * Wahi flow: shift chuno → All/Department scope → only-unassigned ya
#    replace mode → ek tap me assign. Jo shift employee ko assign hogi
#    WAHI Dummy Shift Matrix report me dikhegi. Report-only.
#
# 2) ATTENDANCE POLICY — SIRF OFFLINE SALARY FIRMS (USER REQUEST):
#  * Switch Firm dropdown + "Saved Policies" list ab SIRF un firms ko
#    dikhati hai jinme Firm Master → Offline (Actual) Salary enabled hai.
#  * Baaki firms ka attendance policy se koi link nahi — wo list me
#    aati hi nahi. (406 firms → sirf relevant firms.)
#
# 3) SALARY PROCESS (ARREAR) — FIRM DROPDOWN (USER REQUEST):
#  * "Select firm" ab 400+ chips ki jagah proper DROPDOWN list hai
#    (mobile par search + top matches). Firm chunte hi wahi process hoti hai.
#
# ══════ ALSO IN Iter 768 — 🔗 SHARED MACHINE + 🔄 AUTO M2M SYNC + ☑ 3-POLICY RULE ══════
#
