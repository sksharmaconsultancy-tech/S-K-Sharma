#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 632)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 631-632) ═══════════
#
# 💰 Iter 632 — COMPLIANCE SALARY EXPORTS IN WHOLE RUPEES (user bug):
#  * The exported Excel/CSV Salary Sheet showed paise (Gross Paid
#    2903.85, Net 2881.85 …) while the processed sheet/PDF register
#    show whole rupees — a visible "difference" between processed and
#    exported salary. Every money column (earning heads, gross, wage
#    bases, PF/ESIC/PT/TDS, deductions incl. dynamic Firm-Master
#    heads, net, freeze gross/difference) now exports rounded to the
#    rupee. Days / hours / master rates keep decimals (22.5 days OK).
#  * Stored run data is untouched — rounding is applied at export time.
#
# ⚠️  Iter 631 — ALLOWANCE DISABLE WARNING (user request):
#  * Turning OFF an allowance head in Firm Master → Salary Structure
#    now first checks all processed compliance months for amounts in
#    that head and shows a confirmation with the month-wise total,
#    employee count and FINALIZED status before disabling.
#  * Cancel keeps the toggle ON; saved values are never deleted.
#
# ⚖️  Iter 630 (also included) — Allowance Enable/Disable & Freeze
#    reconciliation contract · 629 — PWA punch fixes · 628 — Dummy
#    Shift Matrix · 627 — Shift Deployment Summary.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy632.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy632.sh
...
echo -n "   Server badge is 632 (must say OK): "
grep -q 'APP_ITERATION = "632"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 632 deployed."
echo "  • NEW (632): COMPLIANCE EXPORTS IN WHOLE RUPEES — the Excel /"
