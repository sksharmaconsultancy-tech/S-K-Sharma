#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 731)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 731) — 💼 ASSET MANAGEMENT MODULE (पूरा lifecycle) ═══════════
#
...
#   wget -O deploy731.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy731.sh
...
grep -q 'APP_ITERATION = "731"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
