#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 687)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 687) — REPORT HUB FIXES (user list) ═══════════
#
...
#   wget -O deploy687.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy687.sh
...
echo -n "   Server badge is 687 (must say OK): "
grep -q 'APP_ITERATION = "687"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
