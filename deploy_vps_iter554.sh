#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 554)
#
# ═══════════ WHAT'S NEW (this deploy = Iter 554) ═══════════
#
# DAILY IN/OUT & OT VERIFICATION REPORT — user-requested changes:
#
# A. 12-HOUR AM/PM TIME OPTION:
#    • New "AM/PM Time" toggle in the filter bar — Punch In/Out and
#      OT In/Out instantly show as 08:30 AM / 07:15 PM.
#    • Carries into Excel, CSV, PDF, Print, Email and WhatsApp exports
#      and the employee drill-down (punch timeline + 7-day history).
#
# B. CENTERED PDF HEADER:
#    • Row 1 (centre): Company Name (+ Group when a Group filter is
#      selected), with address below.
#    • Row 2 (centre): "Daily Report — DD-MM-YYYY (Ddd)".
#
# C. DESIGNATION-WISE PRINT BY DEFAULT:
#    • PDF now groups rows DESIGNATION-wise by default (grey band per
#      designation). Department-wise stays available via the new
#      "PDF Print Grouping" dropdown.
#
# D. EMPLOYEE STATUS FILTER REMOVED:
#    • The report always shows ACTIVE employees only (no dropdown).
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy554.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy554.sh
...
echo -n "   Server badge is 554 (must say OK): "
grep -q 'APP_ITERATION = "554"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
echo -n "   AM/PM backend helper (must say OK): "
grep -q '_to12' $APP_DIR/backend/routes/daily_verification.py && echo "OK" || echo "MISSING!"
echo -n "   AM/PM toggle in UI (must say OK): "
grep -q 'AM/PM Time' $APP_DIR/frontend/app/daily-verification.tsx && echo "OK" || echo "MISSING!"
echo -n "   Centered PDF header (must say OK): "
grep -q 'Daily Report —' $APP_DIR/backend/routes/daily_verification.py && echo "OK" || echo "MISSING!"
echo -n "   Designation-wise default grouping (must say OK): "
grep -q 'group_by == "department"' $APP_DIR/backend/routes/daily_verification.py && echo "OK" || echo "MISSING!"
echo -n "   PDF grouping dropdown in UI (must say OK): "
grep -q 'PDF Print Grouping' $APP_DIR/frontend/app/daily-verification.tsx && echo "OK" || echo "MISSING!"
echo -n "   Employee Status filter REMOVED (must say OK): "
grep -q 'Employee Status' $APP_DIR/frontend/app/daily-verification.tsx && echo "MISSING!" || echo "OK"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 554 deployed."
echo "  Test: Reports → Daily In/Out & OT Verification →"
echo "    1. Flip the new 'AM/PM Time' toggle → times show 08:30 AM."
echo "    2. PDF (Landscape) → header centred: Company (top) +"
echo "       'Daily Report — DD-MM-YYYY' below; DESIGNATION bands."
echo "    3. 'PDF Print Grouping' → Department-wise for the old style."
echo "    4. Employee Status dropdown is gone (active only)."
