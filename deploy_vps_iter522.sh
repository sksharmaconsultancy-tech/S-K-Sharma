#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 522)
#
# ═══════════════════ WHAT'S NEW IN 522 ═══════════════════
#
...
#   wget -O deploy522.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy522.sh
...
echo -n "   Server badge is 522 (must say OK): "
grep -q 'APP_ITERATION = "522"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
