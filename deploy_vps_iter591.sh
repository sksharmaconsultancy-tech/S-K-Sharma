#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 591 — FULL PERMISSION MATRIX + OTP LOGIN FOR CLIENT USERS)
# Deploys the FULL latest code (includes ALL of Iter 568-587). Running 588 alone is enough.
#
# ═══════════ WHAT'S NEW (this deploy = Iter 591) ═══════════
#
# 🔐 ACCESS & WORKFLOW MANAGEMENT — FULL PERMISSION MATRIX:
#  * Roles & Permissions now shows the COMPLETE catalog — 16 modules
#      (Employee Master, Attendance Policy, Attendance Review, Punch/Shift
#      Approvals, Actual Salary, Compliance Salary, PF/ESIC/Challans,
#      Reports, General Masters, Firm Master, Biometric Devices, Portal
#      Credentials, Client Requests, Tickets, Messages, User Management)
#      × ALL 6 actions (View R / Add / Edit W / Delete / Export / Approve).
#  * Every cell is editable. NEW bulk toggles: tap a column header to grant
#      or revoke that action on ALL modules at once; tap a module's ALL to
#      grant/revoke everything for that module. Friendly module names with
#      the technical key underneath.
#
# 🔑 OTP LOGIN (2FA) FOR CLIENT USERS TOO:
#  * Client firm admins and client staff now go through the SAME OTP login
#      as Super/Sub admins: password → OTP to their email → session only
#      after verification. Wrong-OTP lockout, resend cooldown, and audit
#      all apply. Employee PWA logins are unchanged.
#  * Roles list is configurable in Administration → Security 2FA settings
#      (Super/Sub admin 2FA stays permanently mandatory).
#
...
#   wget -O deploy591.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy591.sh
...
echo -n "   Server badge is 591 (must say OK): "
grep -q 'APP_ITERATION = "591"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
