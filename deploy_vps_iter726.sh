#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 726)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 726) — 🌙 NIGHT SHIFT "MISSING PUNCH" FIX + ⚡ SPEED PHASE 2 ═══════════
#
...
#   wget -O deploy726.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy726.sh
...
grep -q 'APP_ITERATION = "726"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
