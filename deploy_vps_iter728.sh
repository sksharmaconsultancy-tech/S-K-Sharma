#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 728)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 728) — ✅ AMIT KUMAR CASE: 24-HOUR DOUBLE DUTY अब सही ═══════════
#
...
#   wget -O deploy728.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy728.sh
...
grep -q 'APP_ITERATION = "728"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
