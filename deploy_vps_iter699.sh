#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 699)
...
#   wget -O deploy699.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy699.sh
...
echo -n "   Server badge is 699 (must say OK): "
grep -q 'APP_ITERATION = "699"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
grep -q '_dup_epfo_login_warning' $APP_DIR/backend/routes/portal_extension.py && grep -q 'EPFO_PC_ACTION' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'GuardedGridInput' $APP_DIR/frontend/app/firm-master.tsx && grep -q 'RUNNER_VERSION = "25"' $APP_DIR/backend/routes/portal_extension.py && echo "OK" || echo "MISSING!"
...
grep -q 'not All Firms' $APP_DIR/frontend/app/pf-reports.tsx && grep -q 'RUNNER_VERSION = "25"' $APP_DIR/backend/routes/portal_extension.py && grep -q 'sks-pwa-v17' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'typeof body === "string" ? body' $APP_DIR/frontend/src/api/client.ts && grep -q 'sks-pwa-v17' $WEB_DIR/sw.js && echo "OK" || echo "MISSING!"
...
grep -q 'ecr_attached' $APP_DIR/backend/routes/portal_extension.py && grep -q 'ecr_attached' $APP_DIR/frontend/app/automation-studio.tsx && grep -q 'RUNNER_VERSION = "25"' $APP_DIR/backend/routes/portal_extension.py && grep -q '"label": "ECR Upload"' $APP_DIR/backend/utils/rpa_engine.py && echo "OK" || echo "MISSING!"
