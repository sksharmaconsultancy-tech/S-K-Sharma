#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 648)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 648) ═══════════
#
# 🐛 BULK EMPLOYEE CORRECTION — Rate/Pay Basis FIX (user bug):
#  * Pay Basis (Actual) edited in Bulk Correction used to APPEND a second
#    "Basic Salary" row while the Employee Master read the older "Basic"
#    row — so the master kept showing MONTHLY. The edit now lands on the
#    existing basic row (any naming), legacy duplicate rows are merged,
#    and the master + bulk screens prefer the row carrying the rate type.
#
# 🧰 Iter 647-644 (also included) — Salary Lock hardened + editable
#    INCENTIVE column · Import-sheet FOOD ALLOWANCE fix · INCENTIVE
#    columns in PDFs/grid/exports · OT toggle columns · 643 earlier fixes.
#
# Run ON THE VPS as root/sksharma:
#   wget -O deploy648.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy648.sh
...
echo -n "   Server badge is 648 (must say OK): "
grep -q 'APP_ITERATION = "648"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 648 deployed."
echo "  • NEW (648): Bulk Correction Rate/Pay Basis now sticks — the"
echo "    Employee Master shows the corrected basis (duplicates merged)."
