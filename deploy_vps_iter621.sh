#!/bin/bash
# S.K. Sharma & Co. — VPS deploy script (Iter 621)
# Deploys the FULL latest code (includes ALL previous iterations).
#
# ═══════════ WHAT'S NEW (Iter 621) ═══════════
#
# 🏷️ PF PRORATION BADGE (user-approved improvement):
...
#   wget -O deploy621.sh "https://emplo-connect-1.preview.emergentagent.com/api/temp-code-bundle?token=sks-deploy-7391&kind=script"
#   bash deploy621.sh
...
echo -n "   Server badge is 621 (must say OK): "
grep -q 'APP_ITERATION = "621"' $APP_DIR/backend/server.py && echo "OK" || echo "MISSING!"
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  DONE — Iter 621 deployed."
echo "  • SALARY GRID: PF in the grid now follows the firm's configured"
echo "    PF Proration Method (e.g. Working Days ÷26) — no more PF"
echo "    changes between Save and Reprocess. Wage Base fills when you"
echo "    type days; deleted sheets no longer offer 'Reprocess'."
echo "  • WHY PF LOOKED WRONG: your Compliance Settings use PF Proration"
echo "    = Working Days (÷26). On a 30-day sheet the earnings divide by"
echo "    30 but PF divides by 26. Want PF = 12% of the earned wage"
echo "    base? Switch to 'Calendar Days' and reprocess."
